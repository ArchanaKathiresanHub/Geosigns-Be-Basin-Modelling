...
FastDepthCalibration::FastDepthCalibration(char* projectFileName, int referenceSurface, int endSurface,
...
void FastDepthCalibration::checkReferenceAndEndSurfaceBoundsAreValid() const
{
...
void FastDepthCalibration::reloadModel()
{
  try
  {
...
void FastDepthCalibration::prepareModelToRunAutomaticDepthCalibration()
{
  try
  {
...
bool FastDepthCalibration::checkForHiatus(const mbapi::StratigraphyManager::SurfaceID currentSurface)
{
...
bool FastDepthCalibration::setTwtMapsIfMissingInBottomSurface()
{
...
void FastDepthCalibration::setDepthAndTwtMapsForNextSurfaceInTables(std::string & mapName, const bool generated_Twt, const std::vector<double> & increasedDepths)
{
  const mbapi::StratigraphyManager::SurfaceID nextSurface = m_fdcLithoProperties.nextSurface();
...
void FastDepthCalibration::runFastCauldronAndCalculateNewDpeths()
{
...
void FastDepthCalibration::writeNewDepthAndCorrectedMapstoCaseFileInMasterDirectory(const std::string & mapName)
{
...
void FastDepthCalibration::calibrateDepths()
{
  ...
  for (mbapi::StratigraphyManager::SurfaceID currentSurface = m_referenceSurface; currentSurface < m_endSurface; ++currentSurface)
  {
    ...
    LogHandler(LogHandler::INFO_SEVERITY) << "Running depth calibration between " << currentTopName << " and " << nextTopName << " for Layer: " << currentLayerName;
    prepareModelToRunAutomaticDepthCalibration();
...
void FastDepthCalibration::calculateNewDepths(const DataAccess::Interface::GridMap * twtGridMap,
...
std::vector<double> FastDepthCalibration::calculateErosion(const mbapi::StratigraphyManager::SurfaceID surfID) const
{
  std::vector<double> erosion;
  //LogHandler(LogHandler::INFO_SEVERITY) << "Running FastDepthCalibration::calculateErosion for surfID " << surfID << "";
