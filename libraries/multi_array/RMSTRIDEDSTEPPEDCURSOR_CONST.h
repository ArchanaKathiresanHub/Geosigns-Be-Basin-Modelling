...
private:
  ...
  RMSTRIDEDSTEPPEDCURSOR<dim_min_1,ENTRY> m_nextDim;
...
public:
  ...
  inline RMSTRIDEDSTEPPEDCURSOR(const Tuple<DIM,int>& steps, const Tuple<DIM,int>& size,
    CONST ENTRY* data):
    ...
    m_nextDim( size.rdc(), steps.rdc(), data )
  {
  ...
  inline RMSTRIDEDSTEPPEDCURSOR(const Tuple<DIM,int>& steps, const Tuple<DIM,int>& size, 
    ...
    m_here( m_start + m_charStep * arrayPos[dim_min_1] ),
    m_finish( m_start + m_charStep * m_arraySizeMin1 ),
    m_nextDim( size.rdc(), steps.rdc(), data, arrayPos.rdc() )
  {
...
#include "multi_array/RMSTRIDEDSTEPPEDCURSOR_CONST_1.h"
