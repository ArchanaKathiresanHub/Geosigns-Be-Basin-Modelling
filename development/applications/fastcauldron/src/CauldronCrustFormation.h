...
inline bool CauldronCrustFormation::isSourceRock () const noexcept {
   return false;
}

inline bool CauldronCrustFormation::isBasalt() const noexcept {
   return isBasaltLayer;
}

inline double CauldronCrustFormation::getHeatProduction ( const unsigned int i,
                                                  const unsigned int j ) const {
   return m_heatProductionMap->getValue ( i, j );
}


inline void CauldronCrustFormation::setBasaltLitho(const int iPosition, const int jPosition, const int kPosition ) {
   if( !BasaltMap.isNull() ) {
      BasaltMap( iPosition, jPosition, kPosition ) = true;
   }
}

inline bool CauldronCrustFormation::getPreviousBasaltLitho(const int iPosition, const int jPosition, const int kPosition ) {
