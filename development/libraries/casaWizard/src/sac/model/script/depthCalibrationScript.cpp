#include "depthCalibrationScript.h"
...
DepthCalibrationScript::DepthCalibrationScript(const SACScenario& scenario, const QString& baseDirectory) :
...
bool DepthCalibrationScript::generateCommands()
{
  if (scenario_.clusterName().toLower() == "local")
  {
    addCommand("fastdepthcalibration"
  ...
  else
  {
    ...
    {
      Logger::log() << "- Failed to write depth calibration script, as file can not be opened" << Logger::endl();
      return false;
    }

    Logger::log() << "- Start writing depth calibration file to " << scriptFilename_ << Logger::endl();
    writeScriptContents(file);
    file.setPermissions(QFile::ReadOwner|QFile::WriteOwner|QFile::ExeOwner|
                        QFile::ReadGroup|QFile::ExeGroup|
                        QFile::ReadOther|QFile::ExeOther);
    file.close();
    Logger::log() << "- Finished writing depth calibration file to " << scriptFilename_ << Logger::endl();
...
void DepthCalibrationScript::writeScriptContents(QFile& file) const
{
  ...
  out << QString("#BSUB -J \"Fastcauldron T2Z calibration run\"\n");
  ...
  out << QString("mpirun_wrap.sh -n " + QString::number(scenario_.numberCPUs()) +
                 " -outfile-pattern 'fastdepthcalibration-output-rank-%r.log'"
