...
BPAClient::BPAClient( QWidget* parent,  const char* name, WFlags fl )
    : BPAClientBase( parent, name, fl ), InfoBase (), m_server (""), m_name (""), m_password ("")
{
   ...
   */
   ...
   if (myServer)
   {
      m_loginDialog->addServer (myServer, myServer);
   }
...
{
   QMessageBox::information (this, "About CauldronBPA",
	 "CauldronBPA is a BPA client for storing and retrieving Cauldron projects,\n"
	 "their input maps and user-selected output maps on a BPA server", QMessageBox::Ok);
}
