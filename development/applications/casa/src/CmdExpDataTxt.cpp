...
template<> void writeOutputStream<std::string>(const std::string myRes, std::ofstream & ofs)
{
   ofs << myRes << " ";
}
...
void CmdExpDataTxt::exportParameters(std::unique_ptr<casa::ScenarioAnalysis>& scenario)
{
   ...
   for (size_t c = 0; c < doeCaseSet.size(); ++c)
   {
      ...
      results.back() = doeCaseSet.runCase(c)->extractParametersValuesAsDouble();
   }
   saveResults(results);
}

template<class T>
void CmdExpDataTxt::saveResults(const std::vector< std::vector<T>>& res, const std::vector<std::string>* headers)
{
   std::ofstream ofs;
   ...
   if(headers != nullptr && res.size() > 0)
   {
      if (headers->size() != res.begin()->size()) throw ErrorHandler::Exception(ErrorHandler::IoError) << "The number of headers doesn't match the data size when trying to write to: " << m_dataFileName;
      for (const auto& header : *headers)
      {
         writeOutputStream(header, ofs);
      }
      ofs << std::endl;
   }
...
void CmdExpDataTxt::exportRunCaseObs(std::unique_ptr<casa::ScenarioAnalysis>& scenario)
{
   ...
   if (doeCaseSet.size() > 0)
   {
      obsNames = doeCaseSet.runCase(0)->extractObservableNames();
   }

   for ( size_t c = 0; c < doeCaseSet.size(); ++c )
   {
      runCasesObservables.push_back(std::vector<double>());
      runCasesObservables.back() = doeCaseSet.runCase(c)->extractObservableValues();
   }

   saveResults(runCasesObservables,&obsNames);
}
...
void CmdExpDataTxt::exportEvalObserv(std::unique_ptr<casa::ScenarioAnalysis>& scenario)
{
   ...
   for (size_t iRunCase = 0; iRunCase < doeCaseSet.size(); ++iRunCase)
   {
      ...
      proxyEvaluationObservables[i] = runCase.get()->extractObservableValues();
      ++i;
   }

   saveResults(proxyEvaluationObservables,&obsNames);
}
...
void CmdExpDataTxt::exportMCResults(std::unique_ptr<casa::ScenarioAnalysis>& scenario)
{
   ...
   columnNames.push_back("Sample_num");
   columnNames.push_back("Sample_RMSE");

   if (mcSamples.size() > 0)
   {
      std::vector<std::string> parNames = mcSamples.begin()->second->extractParameterNames();
      std::vector<std::string> obsNames = mcSamples.begin()->second->extractObservableNames();

      columnNames.insert(columnNames.end(),parNames.begin(),parNames.end());
      columnNames.insert(columnNames.end(),obsNames.begin(),obsNames.end());
   }
   ...
   for (size_t i = 0; i < mcSamples.size(); ++i)
   {
      ...
      std::vector<double> parVals = mcSamples[i].second->extractParametersValuesAsDouble();
      rsmpl.insert(rsmpl.end(),parVals.begin(),parVals.end());

      // extract observables values set
      std::vector<double> obsVals = mcSamples[i].second->extractObservableValues();
