...
///   - casa::BusinessLogicRulesSet - namespace which keeps set of high level function to define influential parameters in scenario analysis
...
///   - casa::VarParameter - @link CASA_VarParameterPage Base class for influential parameter description.@endlink It is inhereted by:
///      -# casa::VarPrmContinuous  - @link CASA_VarPrmContinuousPage Continuous influential parameters @endlink
///      -# casa::VarPrmCategorical - @link  CASA_VarPrmCategoricalPage Influential parameters for categorical values @endlink
///   - casa::VarSpace - @link CASA_VarSpacePage Container which keeps and allows to manipulate influential parameters description.@endlink
///   - casa::RunCase - main class for keeping influential parameters value and targets value for a single Cauldron project run or for
...
///     -# casa::PrmLithoFraction @link CASA_LithoFractionPage Lithology fractions in a mix for a particular layer @endlink
...
///     -# casa::PrmPermeabilityModel - @link CASA_PermeabilityModelPage Lithology permeability model @endlink
...
///     -# casa::PrmCompactionCoefficient - @link CASA_CompactionCoefficientPage compaction coefficient of lithology porosity model parameter @endlink
...
///      -# casa::ObsValueDoubleScalar - Observable value represented by float point scalar value. For example pressure.
///      -# casa::ObsValueDoubleArray - Observable value represented by a list of float point scalar values. For example pressure along well path.
///      -# casa::ObsValueTransformable - @link CASA_ObservableValueTransformable Observable value transformed in some way for better prediction by response surface. @endlink
///
...
/// @page CASA_VarParametersAndObservablesPage Influential Parameters and Observables
...
///
/// Influential parameters are kept by @subpage CASA_VarSpacePage container. Types of implemented in API
/// influential parameters are described here: @link CASA_VarParameterPage @b Influential @b Parameter @endlink
///
...
/// @brief Name space which keeps API for performing uncertainty/sensitivity scenario analysis
namespace casa
{
   /// @brief Name space which keeps set of high level functions for influential parameters definitions.
   ///        This set of functions provides an easy way to add influential parameters to casa::ScenarioAnalysis
   namespace BusinessLogicRulesSet
   {
   template<class Prm>
   ErrorHandler::ReturnCode VaryParameter(
       ...
       , VarPrmContinuous::PDF rangeShape = VarPrmContinuous::Block  /**< [in] defines a type of probability function for the parameter.
                                                                          If PDF needs some middle parameter value it will be taken from
                                                                          the base case model */
       )
   {
     ...
     if ( baseVal < minVal || baseVal > maxVal )
     {
       ...
       return sa.reportError( ErrorHandler::OutOfRangeValue, s.str());
     }

     VarSpace & varPrmsSet = sa.varSpace();
     if ( ErrorHandler::NoError != varPrmsSet.addParameter(new casa::VarPrmContinuousTemplate<Prm>(baseCase, name, minVal, maxVal, rangeShape)) )
     {
     ...
     return ErrorHandler::NoError;
      ...
      ErrorHandler::ReturnCode VaryTopCrustHeatProductionGrid(
      ...
      /// @brief Add 4 influential parameters to one crust thinning event.
      /// @return ErrorHandler::NoError on success or error code otherwise
      ...
      ErrorHandler::ReturnCode VarySurfacePorosity(
          ...
          , const std::string                                      & name          ///< user specified name for influential parameter
      ...
      ErrorHandler::ReturnCode VaryLithoFraction(
         ...
         , casa::VarPrmContinuous::PDF   pdfType            /**< [in] probability function types for the influential parameters. If PDFs need
