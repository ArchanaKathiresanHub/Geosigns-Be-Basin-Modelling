...
void expectFileEq(const std::string& filename1, const std::string& filename2, std::set<int> linesToExclude = {})
{
   ...
   while (keepReading)
   {
      ...
      if (linesToExclude.find(lineNumber) == linesToExclude.end()) //Line not excluded
      {
