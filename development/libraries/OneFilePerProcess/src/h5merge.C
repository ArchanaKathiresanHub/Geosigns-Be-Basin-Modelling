   ...
   case H5G_GROUP:
      {
         ...
         if( reader->m_rank == 0 ) {       
            ...
            startTime = MPI_Wtime();
//               // open a group
//               reader->m_groupId = H5Gopen( reader->m_globalFileId, name, H5P_DEFAULT ); 
...
herr_t updateDataset ( hid_t groupId, const char* name, void * voidReader)  {
   ...
   case H5G_GROUP:
      {
         ...
         // Iterate over the members of the group
         H5Giterate ( reader-> m_localFileId, groupName.str().c_str(), 0, updateDataset, voidReader );
   ...
   if( reader->m_rank == 0 ) {
      ...
      if( !reader->m_reuse ) {
         ...
         if( reader->m_groupId != H5P_DEFAULT ) {
            // Dataset is under the sub-group
            global_dset_id = H5Dopen ( reader->m_groupId, name, H5P_DEFAULT );
            if( global_dset_id < 0 ) {
               global_dset_id = H5Dcreate( reader->m_groupId, name, dtype, memspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
            }
         } else {
            // Dataset is under the main group
            global_dset_id = H5Dopen ( reader->m_globalFileId, name, H5P_DEFAULT ); 
            if( global_dset_id < 0 ) {
               global_dset_id = H5Dcreate( reader->m_globalFileId, name, dtype, memspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
            }
         }
...
bool appendFiles( MPI_Comm comm, const std::string & fileName, const std::string & tempDirName ) {
   ...
   if( reader.m_rank == 0 ) {
      ...
      reader.m_globalFileId = H5Fopen( fileName.c_str(), H5F_ACC_RDWR, H5P_DEFAULT ); 
   ...
   iteration_status = H5Giterate ( reader.m_localFileId, "/", 0, updateDataset, &reader );
