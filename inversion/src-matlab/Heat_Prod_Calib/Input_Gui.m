...
% INPUT_GUI M-file for Input_Gui.fig
...
% Last Modified by GUIDE v2.5 21-Sep-2011 10:01:27
...
set(handles.VariableTable, 'ColumnFormat', {[] [] [] [] {'Single' 'Multi'} [] [] [] ...
       [] []});
...
if (eventdata.Indices(2) == 3 && ...
       (strcmp(eventdata.NewData,'Single')))
       tableData = get(hObject, 'data');
       tableData{eventdata.Indices(1), eventdata.Indices(2)+1} = 1;
       tableData{eventdata.Indices(1), eventdata.Indices(2)+2} = 1;
       set(hObject, 'data', tableData);
    
end
   

if (eventdata.Indices(2) == 4 && strcmp(tableData{eventdata.Indices(1), eventdata.Indices(2)-1},'Single') && (eventdata.NewData ~=1))
       tableData = get(hObject, 'data');
       tableData{eventdata.Indices(1), eventdata.Indices(2)} = 1;
      warndlg('For single, only one point','Warning');
       set(hObject, 'data', tableData);    
end
 if (eventdata.Indices(2) == 5 && strcmp(tableData{eventdata.Indices(1), eventdata.Indices(2)-2},'Single') && (eventdata.NewData ~=1))
       tableData = get(hObject, 'data');
       tableData{eventdata.Indices(1), eventdata.Indices(2)} = 1;
      warndlg('For single, only one point','Warning');
       set(hObject, 'data', tableData);    
...
%save input data to a file
                inputDataFile = [Input.OutputDir Input.BaseName '_InputData_bkup.dat'];
                finput = fopen(inputDataFile,'w');
% save data files
                variableFile = [Input.OutputDir Input.BaseName '_Variables.dat'];
                fvar = fopen(variableFile,'w');
                % save data files
                calibPointFile = [Input.OutputDir Input.BaseName '_CalibrationPoints.dat'];
                fcalibPoint = fopen(calibPointFile,'w');
                calibParamFile = [Input.OutputDir Input.BaseName '_CalibrationParams.dat'];
                ...
                fprintf(finput,'%s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\n', 'Type', 'CauldronName', 'VariableName', 'Name', 'PointType', 'NoOfPoints','PointToChange','Min', 'Max', 'Initial');
                       for n = 1:rows
                           Input.InversionParameters(n).MainVariableType=tableData{n, 1} ;
                            Input.InversionParameters(n).LithologyName =tableData{n, 2} ;
                            Input.InversionParameters(n).CauldronName = tableData{n, 3} ;
                            Input.InversionParameters(n).Name = tableData{n, 4} ;
                            Input.InversionParameters(n).SingleOrMulti = tableData{n, 5} ;                       
                            Input.InversionParameters(n).MultiPoints = str2double(tableData{n, 6});
                            Input.InversionParameters(n).MultiPointToChange = str2double(tableData{n, 7} );
                            Input.InversionParameters(n).Min = str2double(tableData{n, 8} );
                            Input.InversionParameters(n).Max = str2double(tableData{n, 9} );
                            Input.InversionParameters(n).Initial = str2double(tableData{n, 10} );
                            ...
                            fprintf(fvar,'%s\t %s\t %s\t %s\t %s\t %g\t %g %g\t %g\t  %g\n',Input.InversionParameters(n).MainVariableType,   Input.InversionParameters(n).LithologyName, Input.InversionParameters(n).CauldronName,Input.InversionParameters(n).Name, Input.InversionParameters(n).SingleOrMulti,...
                                Input.InversionParameters(n).MultiPoints,Input.InversionParameters(n).MultiPointToChange,Input.InversionParameters(n).Min,Input.InversionParameters(n).Max,Input.InversionParameters(n).Initial);
                            fprintf(finput,'%s\t %s\t %s\t %s\t %s\t %g\t %g\t %g\t %g\t  %g\n',Input.InversionParameters(n).MainVariableType, Input.InversionParameters(n).LithologyName,  Input.InversionParameters(n).CauldronName, Input.InversionParameters(n).Name, Input.InversionParameters(n).SingleOrMulti,...
                                Input.InversionParameters(n).MultiPoints,Input.InversionParameters(n).MultiPointToChange,Input.InversionParameters(n).Min,Input.InversionParameters(n).Max,Input.InversionParameters(n).Initial);
                        end
