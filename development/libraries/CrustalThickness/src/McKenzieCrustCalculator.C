      ...
      m_validator    ( validator  )
{
   ...
   else if (m_constants.getModelTotalLithoThickness() == 0){
   ...
   ) const
{
   ...
   theFunction.setWLS_crit ( TTScritical        );
   theFunction.setWLS_onset( TTSOnsetLinearized );
...
//------------------------------------------------------------//
double McKenzieCrustCalculator::calculateContinentalCrusltalThickness( const double thinningFactor, const double initialContinentalCrustThickness ) const {
   double continentalCrusltalThickness = 0;
   if ( thinningFactor <= 1.0 ){
      continentalCrusltalThickness = (1 - thinningFactor) * initialContinentalCrustThickness;
   }
   return continentalCrusltalThickness;
}

//------------------------------------------------------------//
double McKenzieCrustCalculator::calculateOceanicCrustalThickness( const double TTS, const double TTSexhume, const double TTSadjusted, const LinearFunction& linearFunction ) const {
   double oceanicCrustalThickness = 0;
   if ( TTS < TTSexhume ){
      oceanicCrustalThickness = linearFunction.getBasaltThickness( TTSadjusted );
   }
...
void McKenzieCrustCalculator::compute(){
   ...
   unsigned int i, j;
   double continentalCrustDensity, potentialTempAstheno, magmaticDensity;
   double thinningFactorOnset, thinningFactorOnsetLinearized, thinningFactorOnsetAtMaxBasalt;
   double TTSexhume, TTScritical, TTSOnsetLinearized, TTSexhumeSerpentinized;
   double thicknessContinetalCrustOnset, residualDepthAnomaly, thinningFactor;
   double continentalCrusltalThickness, oceanicCrustalThickness, topOceanicCrust, moho, effectiveCrustalThickness;
   ...
   for (i = m_firstI; i <= m_lastI; ++i) {
      for (j = m_firstJ; j <= m_lastJ; ++j) {
             ...
             endRift                            != m_TRMap.getUndefinedValue() )
         {
            ...
            const double TTSadjusted   = m_outputData.getMapValue( CrustalThicknessInterface::outputMaps::WLSadjustedMap, i, j );
            ...
            if ( TTSadjusted != Interface::DefaultUndefinedMapValue and
                 TTS != Interface::DefaultUndefinedMapValue and
                 TTScritical >= TTSOnsetLinearized ){
               residualDepthAnomaly         = calculateResidualDepthAnomaly        ( TTScritical, TTSadjusted );
               thinningFactor               = linearFunction.getCrustTF            ( TTSadjusted );
               continentalCrusltalThickness = calculateContinentalCrusltalThickness( thinningFactor, initialContinentalCrustThickness );
               oceanicCrustalThickness      = calculateOceanicCrustalThickness     ( TTS, TTSexhume, TTSadjusted, linearFunction );
               topOceanicCrust              = calculateTopOceanicCrust             ( continentalCrusltalThickness, depthBasement );
               moho                         = calculateMoho                        ( topOceanicCrust, oceanicCrustalThickness );
               effectiveCrustalThickness = calculateEffectiveCrustalThickness( continentalCrusltalThickness,
               ...
               continentalCrusltalThickness = Interface::DefaultUndefinedMapValue;
         ...
         m_outputData.setMapValue( CrustalThicknessInterface::outputMaps::thicknessCrustMap,        i, j, continentalCrusltalThickness   );
