    ...
    : ProjectViewBase( parent, name, fl )
{
   ...
   m_exportButton->hide ();
   m_reexportButton->hide ();
   fileConnectBPAAction->removeFrom (fileMenu);
...
{
#if USEMOZILLA
   char * browserName = "mozilla ";
#else
   char * browserName = "netscape ";
#endif
   ...
   if (system (command) == 0)
   {
	 ...
	 strcat(command, dpy);
      ...
      strcat(command, " -remote 'openFile(");
      where = getenv("IBS_NAMES");
      strcat(command, where);
      strcat(command, "/HTML/cauldronmaps.html,new-window)' &");
      // strcat(command, "> /dev/null 2>&1 &");
   ...
   else
   {
	 ...
	 strcat(command, dpy);
      ...
      where = getenv("IBS_NAMES");
      strcat(command, where);
      strcat(command, "/HTML/cauldronmaps.html &");
      // strcat(command, "> /dev/null 2>&1 ");
      // cerr << "executing: " << command << endl;
      system(command);
   }
...
void ProjectView::displayInputValue (const InputValue * iv, GridMapView * gmv)
{
   ...
   if (!gm)
   {
      cerr << "Could not load gridmap for: " << title.ascii () << endl;
   }
...
void ProjectView::getGridMapDescription (const GridMap * gm , QString & description)
{
   if (gm)
   {
      ...
      description = description.append ("GridMap: \n"
	    "NumX = %1, NumY = %2,\n"
	    "DeltaX = %3, DeltaY = %4,\n"
	    "MinX = %5, MinY = %6,\n"
	    "MaxX = %7, MaxY = %8,\n");
      ...
      description = description.arg (g->deltaI ());
      description = description.arg (g->deltaJ ());
      ...
      description = description.append ("Min = %1, Max = %2,\n"
	    "Average = %3");
      gm->getMinMaxValue (min, max);
      description = description.arg (min);
      description = description.arg (max);
      description = description.arg (gm->getAverageValue ());
   }
   else
   {
