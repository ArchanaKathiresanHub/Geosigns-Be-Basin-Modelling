...
void Prograde::UpgradeManagers::runAll() const{
  ...
  managers.emplace_back(std::unique_ptr<Prograde::PorosityUpgradeManager>(new Prograde::PorosityUpgradeManager(m_model)));
   ...
   //17. User defined snapshot table filled by user defined snapshots in SnapshotIoTbl
   managers.emplace_back(std::unique_ptr<Prograde::UserDefinedSnapshotUpgradeManager>(new Prograde::UserDefinedSnapshotUpgradeManager(m_model)));
