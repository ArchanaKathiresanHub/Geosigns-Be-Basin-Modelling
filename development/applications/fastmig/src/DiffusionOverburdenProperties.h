...
private:
   ...
   std::vector<DiffusionLeak::OverburdenProp> m_diffusionOverburdenProps;
   double m_sealFluidDensity;

public:

   DiffusionOverburdenProperties(const std::vector<DiffusionLeak::OverburdenProp>& properties,
      const double& sealFluidDensity):
      m_diffusionOverburdenProps(properties),
      m_sealFluidDensity(sealFluidDensity)
   {}

   const std::vector<DiffusionLeak::OverburdenProp>& properties() const { return m_diffusionOverburdenProps; }
