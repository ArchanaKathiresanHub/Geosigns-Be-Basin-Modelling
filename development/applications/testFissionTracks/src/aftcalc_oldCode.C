...
AftCalc::AftCalc (BOOLEAN_t alloc)
{
   Current = this;
   AftCalc_c aftcalc = this;

   (aftcalc->CALCENGINE) = CalcEngine::GetCurrent ();
   if (!((aftcalc->FtSamples) = ListCreate ()))
   {
      aftcalc->Delete ();
      return;
   }
   ListSetDestroyMethod ((aftcalc->FtSamples), (DestroyMethod) FtSample_Delete);
   if (!((aftcalc->C0) = PropDoubleVector::Create (alloc)))
   {
      aftcalc->Delete ();
      return;
   }
   if (!((aftcalc->C1) = PropDoubleVector::Create (alloc)))
   {
      aftcalc->Delete ();
      return;
   }
