...
// forward elimination
   ...
   for (k = 0; k < N; ++ k ) {
      ...
      maxIndx = k;
      for (i = k; i < N; ++ i) {
         value = fabs(A[indx[i]][k]) / scale[indx[k]];
         if (value > maxRatio) {
  ...
  // Steps (1) and (2) (decomposition and solution of Ly = b)
   ...
   case 1: // Case in which pivoting is employed
      ...
      for(k = 0; k < N-1; ++ k ) {
         ...
         A.RowSwap(pivot,k);
         Swap(b(pivot), b(k)); 
         double invA_kk = 1 / A(k,k);
         for(int i = k + 1; i < N; ++ i) {
            double l_ik = A(i,k) * invA_kk;
            for(int j = k; j < N; ++ j) {
               A(i,j) -=  l_ik * A(k,j);
            }
            b(i) -= l_ik * b(k); 
...
void BiCGStab(const SCMatrix &in_A, const SCVector &in_b, SCVector &x) {
   ...
   double * memoryForAllVectors = new double [N * 9];
   
   double *B = memoryForAllVectors;
   double *result = B + N;
   double * s = result + N;
   double * t = s + N;
   double * vi = t + N;
   ...
   double * ri = pi + N;
   double * roc = ri + N;
   double * half_result = roc + N;

   memset(memoryForAllVectors, 0, sizeof(double) * 9 * N);
   ...
   for( i = 0; i < N; ++ i) {
    ...
    delta0 = norm2(B, N);
    roim1 = dotProduct(roc, ri, N); 

    while( iflag != 0 && icount < maxit ) {

       ++ icount;

       matrixVectorProduct(vi, A, pi, N);  
       al = roim1 / dotProduct( vi, roc, N );  

       vva(half_result, result, 1, pi, al, N);
       matrixVectorProduct(s, A, half_result, N);  
       vv(s, B, 1,  N);
       deltai = norm2(s, N);
       
       if( deltai < EPS * delta0 ) {
          iflag = 0;
          memcpy(result, half_result, N * sizeof(double));
          break;
       }
       vva(s, ri, 1, vi, - al, N);
       matrixVectorProduct(t, A, s, N);
       wi = dotProduct(t, s, N) / dotProduct(t, t, N);  
       vvaPlus(result, pi, al, s, wi, N);
       vva(ri, s, 1, t, -wi, N);
       roi = dotProduct(roc, ri, N); 

       bet = ( roi / roim1 ) * ( al / wi );  
       vvaBig(pi, ri, vi, wi, bet, N);
    ...
    for (i = 0; i < N; ++ i) x(i) = result[i];

   DestroyMatrix(A, N);
   delete [] memoryForAllVectors;
}
