	...
	: Interface::Trap(projectHandle, record)
{
	...
	m_brineSalinity = Utilities::Numerical::IbsNoDataValue;

	auto formations = projectHandle.getFormations();
	...
	double viscosity[ComponentManager::NUMBER_OF_PHASES])
{
	...
	{
		buoyancy += (brineDensity - density[ComponentManager::VAPOUR]) * AccelerationDueToGravity * (getGOC() - getDepth());
	}
	if (massTotal[ComponentManager::LIQUID] > 1)
	{
		buoyancy += (brineDensity - density[ComponentManager::LIQUID]) * AccelerationDueToGravity * (getOWC() - getGOC());
	}
...
void TrackTrap::saveStockTankChargeProperties(database::Record* record, int phaseRC, double masses[ComponentManager::NUMBER_OF_SPECIES], double density, double viscosity)
{
	...
	performPVT(masses, StockTankTemperatureC, StockTankPressureMPa, phaseMassesST, densitiesST, viscositiesST);
