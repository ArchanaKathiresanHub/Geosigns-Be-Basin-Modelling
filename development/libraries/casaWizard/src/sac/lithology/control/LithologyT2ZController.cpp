...
#include "LithologyT2ZController.h"
...
LithologyT2Zcontroller::LithologyT2Zcontroller(T2Ztab* t2zTab,
...
void LithologyT2Zcontroller::slotPushButtonSACrunT2ZClicked()
{
...
bool LithologyT2Zcontroller::noProjectAvailable() const
{
...
bool LithologyT2Zcontroller::runCanceled() const
{
...
bool LithologyT2Zcontroller::referenceSurfaceHasNoTWTData() const
{
   return !casaScenario_.projectReader().hasTWTData(casaScenario_.t2zReferenceSurface());
}

bool LithologyT2Zcontroller::projectHasNoTWTDataBelowReferenceSurface() const
{
   return casaScenario_.projectReader().lowestSurfaceWithTWTData() <= casaScenario_.t2zReferenceSurface();
}

bool LithologyT2Zcontroller::prepareT2ZWorkSpace()
{
   getSourceAndT2zDir();
...
void LithologyT2Zcontroller::getSourceAndT2zDir()
{
   sourceDir_ = "";
   t2zDir_ = "";
...
void LithologyT2Zcontroller::setSubSampling()
{
...
void LithologyT2Zcontroller::runDepthConversion()
{
...
void LithologyT2Zcontroller::slotOutputFile()
{
...
void LithologyT2Zcontroller::slotComboBoxReferenceSurfaceValueChanged(int referenceSurface)
{
   casaScenario_.setT2zReferenceSurface(referenceSurface);
}

void LithologyT2Zcontroller::slotSpinBoxSubSamplingValueChanged(int subSampling)
{
   casaScenario_.setT2zSubSampling(subSampling);
}

void LithologyT2Zcontroller::slotComboBoxProjectSelectionTextChanged(const QString& projectSelection)
{
   casaScenario_.setT2zRunOnOriginalProject(projectSelection == "Original Project");
}

void LithologyT2Zcontroller::slotSpinBoxNumberOfCPUsValueChanged(int numberOfCPUs)
{
   casaScenario_.setT2zNumberCPUs(numberOfCPUs);
}

void LithologyT2Zcontroller::slotComboBoxClusterSelectionTextChanged(const QString& clusterName)
{
   casaScenario_.setClusterName(clusterName);
}

void LithologyT2Zcontroller::slotExportT2ZScenarioClicked()
{
   getSourceAndT2zDir();
   ...
   const QString exportPath = functions::exportScenarioToZip(sourceDir, casaScenario_.workingDirectory(), casaScenario_.project3dFilename(), t2zInfoGenerator);
   CalibrationTargetSaver saver(casaScenario_.calibrationTargetManager(), exportPath);
   saver.saveRawLocationsToText("IncludedWells", ' ', true);
}

void LithologyT2Zcontroller::slotExportT2ZMapsToZycorClicked()
{
   getSourceAndT2zDir();
   const QStringList iterations = QDir(t2zDir_).entryList({"T2Zcal_*"}, QDir::Filter::Dirs, QDir::Time | QDir::Reversed);
   if (iterations.empty())
   ...
   QString targetPath = QFileDialog::getExistingDirectory(nullptr, "Choose a location for the exported zycor maps", casaScenario_.defaultFileDialogLocation(),
   ...
   CalibrationTargetSaver saver(casaScenario_.calibrationTargetManager(), targetDir.absolutePath());
   saver.saveRawLocationsToText("IncludedWells", ' ', true);
}

void LithologyT2Zcontroller::slotUpdateTabGUI(int tabID)
{
...
void LithologyT2Zcontroller::updateSurfaces()
{
   QStringList surfaces = casaScenario_.projectReader().surfaceNames();
   t2zTab_->setReferenceSurfaces(surfaces);
}

void LithologyT2Zcontroller::updateProjectSelectionOptions()
{
