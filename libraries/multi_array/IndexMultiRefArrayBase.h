#ifndef _MULTI_ARRAY_INDEXMULTIREFARRAYBASE_H_
#define _MULTI_ARRAY_INDEXMULTIREFARRAYBASE_H_
...
protected:
  ...
  inline IndexMultiRefArrayBase()
  {}

  inline IndexMultiRefArrayBase(const CloseOpenInterval<DIM,int>& dmn,
    const INDEXMAP& indxMp):
    MultiArrayBase<DIM,typename INDEXMAP::type>(dmn),
    m_indexMap(indxMp)
  {}
...
public:
  ...
  inline IndexMultiRefArrayBase(const IndexMultiRefArrayBase& other):
    RefArrayBase<ENTRY>(other),
    MultiArrayBase<DIM,typename INDEXMAP::type>(other),
