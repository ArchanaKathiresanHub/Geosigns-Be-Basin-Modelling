...
void WellPrepController::slotPushSelectCalibrationClicked()
{
   ...
   if (fileNames.size() > 1)
   {
      for (const QString& fileName : fileNames)
      {
         ...
         {
            messageBox::showErrorMessageBox("Bulk import is only supported for las files");
            return;
         }
      }
   ...
   {
      messageBox::showErrorMessageBox("Unknown error occurred during import");
      return;
   }
...
void WellPrepController::slotExportToCSVClicked()
{
   ...
   QString fileName = QFileDialog::getSaveFileName(wellPrepTab_, "Save as", casaScenario_.defaultDirectoryLocation(), "comma seperated value file (*.csv)");
   if (fileName.isEmpty())
   {
      return;
   }
...
void WellPrepController::slotPushSaveDataClicked()
{
   ...
   QString fileName = QFileDialog::getSaveFileName(wellPrepTab_, "Save as", casaScenario_.defaultDirectoryLocation(), "Spreadsheet (*.xlsx)");
   if (fileName == "")
   {
      return;
   }
