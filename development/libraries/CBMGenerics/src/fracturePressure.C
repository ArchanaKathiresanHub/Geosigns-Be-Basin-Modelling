   ...
   const vector<double>& lithFracs)
{
   ...
   double result = 0.0;
   for (int l = 0; l < lithFracs.size(); ++l)
   {
...
double compute(FracturePressureFunctionType type, const vector<double>& 
   fracturePressureFunctionParameters, const vector<double>& lithHydraulicFracturingFracs, 
   const vector<double>& lithFracs, const double& depthWrtSeaLevel, 
   const double& depthWrtSedimentSurface, const double& lithostaticPressure, 
   const double& hydrostaticPressure, const double& hydrostaticPressureAtSedimentSurface)
{
   double pressure;
   switch (type) {
      case None:
	 ...
	 pressure = computeForFunctionOfDepthWrtSeaLevelSurface(fracturePressureFunctionParameters, 
	    depthWrtSeaLevel);
	 break;
	 ...
	 pressure = computeForFunctionOfDepthWrtSedimentSurface(fracturePressureFunctionParameters, 
	    depthWrtSedimentSurface, hydrostaticPressureAtSedimentSurface);
	 break;
	 ...
	 pressure = computeForFunctionOfLithostaticPressure(hydraulicFracture, 
	    lithostaticPressure, hydrostaticPressure); }
	 break;
      default:
	 assert(0);
   }
   ...
   const double& depthWrtSeaLevel)
{
   ...
   double pressure = (( coefficients[3] * depthWrtSeaLevel + coefficients[2] ) * depthWrtSeaLevel + 
      coefficients[1] ) * depthWrtSeaLevel + coefficients[0];
...
{
   assert(coefficients.size() == 4);
   double pressure = (( coefficients[3] * depthWrtSedimentSurface + coefficients[2] ) * depthWrtSedimentSurface + 
      coefficients[1]) * depthWrtSedimentSurface + coefficients[0];
