/**
 * \file WalderhaugCompactionCalculatorTest.cpp
...
public:
         ...
         double temp1 )
   {
      ...
      double result = - ( ( 1 - clayCoating ) * 6 * fractionQuartz / grainSize );
      result *= ( time0 - time1 ) * MillionYearToSecond * MolarMassQuartz * 1.98e-22 / DensityQuartz;
...
TEST_F( testWalderhaugCompactionCalculator, test_extreme_time_values )
{
   ...
   const double clayCoating    [numberLithologies] = {0.1};  //[fraction of available quartz surface]
   ...
   MockGrid previousGrid( size, currentTime, currentTime, chemicalCompaction1, porosity, previousTemperature, currentTemperature, sizeValidNodes, validNodes,
         lithoId, numberLithologies, fractionQuartz, grainSize, clayCoating );
   ...
   double result = chemicalCompaction2[0] + computeTestTimeStep( clayCoating[0], fractionQuartz[0], grainSize[0],
...
TEST_F( testWalderhaugCompactionCalculator, test_extreme_parameters_values )
{
   ...
   for ( int index = 0; index < size; index += 3 )
   {
      ...
      {
      case 0:
         previousTemperature[index    ] = -273.15;
         previousTemperature[index + 1] = -273.15;
         previousTemperature[index + 2] = -273.15;
      ...
      case 3:
         ...
         currentTemperature [index    ] = -273.15;
         currentTemperature [index + 1] = 120.0;
         currentTemperature [index + 2] = 14000.0;
         break;
   ...
   for ( int i=0 ; i<size; ++i )
   {
      ...
      {
         expectedResult += computeTestTimeStep( clayCoating[0], fractionQuartz[0], grainSize[0],
...
TEST_F( testWalderhaugCompactionCalculator, test_extreme_lithologies_values )
{
   ...
   std::fill_n( porosity,                 size,      0.3 ); //[fraction of unit volume]
   ...
   double clayCoating    [numberLithologies]; //[fraction of available quartz surface]
   ...
   for ( int i=0 ; i<size; ++i )
   {
      ...
      if ( currentTemperature[i] > 80.0 )
      {
         expectedResult += computeTestTimeStep( clayCoating[i], fractionQuartz[i], grainSize[i],
               previousTime, currentTime,
               previousTemperature[i], currentTemperature[i] );
      }
      expectedResult += chemicalCompaction2[i];
...
TEST_F( testWalderhaugCompactionCalculator, test_initial_porosity )
{
   ...
   double expectedResult2 = computeTestTimeStep( clayCoating[0], fractionQuartz[0], grainSize[0],
   ...
   double expectedResult3 = computeTestTimeStep( clayCoating[0], fractionQuartz[0], grainSize[0],
         previousTime1, currentTime,
...
TEST_F( testWalderhaugCompactionCalculator, test_interaction_temperatures )
{
   ...
   const double porosity      [size] = {0.3,0.3,0.3}; //[fraction of unit volume]

   const double previousTemperature [size] = {100.0,100.0,100.0};                  //[K]
   const double currentTemperature  [size] = {100.0,100.000000001,99.9999999990}; //[K]
   ...
   static const double ln10 = std::log( 10.0 );
   double expectedResult = - ( ( 1 - clayCoating[0] ) * 6 * fractionQuartz[0] / grainSize[0] );
   expectedResult *= ( previousTime - currentTime ) * MillionYearToSecond * MolarMassQuartz * 1.98e-22 / DensityQuartz;
   expectedResult *= exp( 0.022 * ( previousTemperature[0] ) * ln10 );
