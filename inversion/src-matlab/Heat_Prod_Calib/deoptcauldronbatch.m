...
%function [FVr_bestmem,S_bestval,I_nfeval] = deoptcauldronbatch(S_struct,Input)
function  deoptcauldronbatch(S_struct,Input)
...
filename1 = [Input.OutputDir Input.BaseName '_Convergence_history_file1'];
filename2 = [Input.OutputDir Input.BaseName '_Convergence_history_file2'];
fid33 = fopen(filename1','w');
fid44 = fopen(filename2,'w');
...
for k=1:I_NP   % Draw the random variables for k sample points
   FM_pop(k,:) = FVr_minbound + rand(1,I_D).*(FVr_maxbound - FVr_minbound);
end


% initialise with given values for first run in a batch
fprintf(fid33, 'Initial values given are: \n');
for k=1:I_D   
   FM_pop(1,k) = Input.InversionParameters(k).Initial;
   fprintf(fid33, '%d : %g \n', k, FM_pop(1,k));
end
fprintf(fid33, 'random values of the population: \n');
for k=1:I_NP   % Draw the random variables for k sample points
   fprintf(fid33, '%d : %g \n', k, FM_pop(k,:));
end
...
%-------------------------------------------------------------------------
% Clone the mother project 3D file and add the calibration locations
% to the project3d file
%------------------------------------------------------------------------
%inside filldataminingtable function, calling fcopylinesuntilkeyword(stringSearch, ref_projectfile_p,new_projectfile_p ); 
[Calibrationdata] = filldataminingtable(Input);
fprintf('Finished writing the calibration data to the data mining table \n');

%------Evaluate the best member after initialization----------------------

I_best_index   = 1;                   % start with first population member
%S_val(1)       = feval(fname,FM_pop(I_best_index,:),S_struct);
fprintf('Launching Initialisation sample number  \n');

%--------------------------------------------------------------------------
% Launching CauldronBatch and return a simple array with corresponding 
% cost values
%--------------------------------------------------------------------------


fprintf('Initialising S_struct.worstmisfit  \n');
for(param=1:Input.NCalibParam)
    S_struct.WorstMisFit(param) = 0.0;
end

batch_id    = 1;
best_fit_k  = 1;
S_struct.batch_winner = batch_id;
S_struct.best_fit_k   = best_fit_k;

fprintf('Evaluate cauldron de batch \n');
[Misfit S_struct] = evaluatecauldrondebatch(FM_pop,S_struct,batch_id,Calibrationdata, Input);

I_best_index   = 1; 
fprintf('Setting Initialisation sample number 1 as starting best match \n');
%take first run misfit 
S_val(1)  = returnmisfitstructure(Misfit,1);
S_bestval = S_val(1);                 % best objective function value so far
I_nfeval  = I_nfeval + 1;
fprintf(fid33, 'Initialisation, S_val.FVr_oa(1) = %f \n',S_val(1).FVr_oa(1));
...
for k=2:I_NP                          % check the remaining members
  ...
  %fprintf('Launching Initialisation sample %d \n',k );
  S_val(k) = returnmisfitstructure(Misfit,k);
  fprintf(fid33, 'Initialisation, S_val(%d).FVr_oa(1) = %f \n',k,S_val(k).FVr_oa(1));
  I_nfeval  = I_nfeval + 1;
  if (left_win(S_val(k),S_bestval) == 1)
     I_best_index   = k;              % save its location
     S_bestval      = S_val(k);
     fprintf(fid33, 'Initialisation, S_struct.FVr_oa(1)= %f \n',S_bestval.FVr_oa(1));
  end   
end
...
fprintf(fid33, 'Initialisation, FVr_bestmem= %f \n',FVr_bestmem);
%------DE-Minimization---------------------------------------------
...
I_iter = 1;
fprintf(fid33,'Optimisation going to start.. , S_bestval.FVr_oa(1): %f, F_VTR: %f \n',S_bestval.FVr_oa(1), F_VTR);
if(S_bestval.FVr_oa(1) < F_VTR)
  fprintf(fid33,'STOP.. found solution\n');  
  fprintf('STOP! found solution \n');
end

while ((I_iter < I_itermax) && (S_bestval.FVr_oa(1) > F_VTR))
  fprintf('I_iter = %d, S_bestval.Fvr_oa(1) = %f \n',I_iter,S_bestval.FVr_oa(1) ); 
  fprintf(fid33, '======> I_iter = %d, S_bestval.Fvr_oa(1) = %f \n',I_iter,S_bestval.FVr_oa(1) ); 
...
% cost values
%-----------------------------------------------------------------------
fprintf('Launching optimization batch number %d \n',batch_id)
fprintf(fid33,'Population before\n');
for i=1:size(FM_ui,1)
   fprintf(fid33, '%f ', FM_ui(i,:));
   fprintf(fid33, '\n');
end


% Put here by Quintijn - otherwise we pass on incorrect values to Cauldron
for k=1:I_NP
   if (I_bnd_constr == 1)
...
fprintf(fid33,'Population after correcting for min and max bounds \n');
for i=1:size(FM_ui,1)
   fprintf(fid33, '%f ', FM_ui(i,:));
   fprintf(fid33, '\n');
end
...
batch_id = batch_id + 1;
[Misfit S_struct] = evaluatecauldrondebatch(FM_ui,S_struct,batch_id, Calibrationdata,Input);
...
%       end
      ...
      S_tempval = returnmisfitstructure(Misfit,k);
      %S_tempval = feval(fname,FM_ui(k,:),S_struct);   % check cost of
      %competitor (the batch has run above!)
      
      fprintf('Launching sample %d \n',k ); 
      I_nfeval  = I_nfeval + 1;
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0)
     if ((rem(I_iter,I_refresh) == 0) || I_iter == 1)
       ...
       cme = S_bestval.FVr_oa(1);
       fprintf(fid33,'Iteration: %d,  Best: %f,  F_weight: %f,  F_CR: %f,  I_NP: %d\n',I_iter,S_bestval.FVr_oa(1),F_weight,F_CR,I_NP);
       fprintf(fid44,'%d %f \n', I_iter, S_bestval.FVr_oa(1));
       %var(FM_pop)
       format long e;
       for n=1:I_D
          fprintf(1,'name = %s, best(%d) = %g\n',S_struct.FVr(n).name,n,FVr_bestmem(n));
          fprintf(fid33,'name = %s, best(%d) = %g\n',S_struct.FVr(n).name,n,FVr_bestmem(n));
       end
       
       fprintf(fid33,'    \n');
       if (I_plotting == 1)
          S_struct = plotitcauldron(Misfit,cme,FVr_bestmem,I_iter,S_struct, Calibrationdata,Input);   % plot convergence and well path for cauldron
       end
    end
  end
...
fprintf('END..! Completed Auto Calibration \n');
fclose(fid33); %Close convergence history file
fclose(fid44);
