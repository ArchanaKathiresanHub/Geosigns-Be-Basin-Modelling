#ifndef _FASTCAULDRON__SUBDOMAIN_ELEMENT__H_
#define _FASTCAULDRON__SUBDOMAIN_ELEMENT__H_
...
public :
   ...
   // /// \brief Set whether or not the element-boundary is on the domain-boundary.
   // ///
   // /// Only GAMMA_1 and GAMMA_6 are valid values here.
   // void setOnDomainBoundary ( const VolumeData::BoundaryId id,
   //                            const bool                   value );
   ...
   const SubdomainElement* getNeighbour ( const VolumeData::BoundaryId id ) const;
   ...
   const SubdomainElement* getActiveNeighbour ( const VolumeData::BoundaryId id ) const;
...
private :
   ...
   ///
   /// May return null if element does not exist.
   const SubdomainElement* getShallowerActiveNeighbour () const;

   /// \brief Return the active neighbouring element deeper than current element.
   ///
   /// May return null if element does not exist.
   const SubdomainElement* getDeeperActiveNeighbour () const;
   ...
   const SubdomainElement* m_neighbours [ NumberOfBoundaries ];
...
inline int SubdomainElement::getI () const {
   return m_layerElement->getIPosition ();
}

inline int SubdomainElement::getJ () const {
...
inline int SubdomainElement::getNodeJ ( const int node ) const {
   return getLayerElement ().getNodeJPosition ( node );
}

inline int SubdomainElement::getNodeK ( const int node ) const {
   return m_nodeKValues [ node ];
}

inline const SubdomainElement* SubdomainElement::getNeighbour ( const VolumeData::BoundaryId id ) const {
   return m_neighbours [ id ];
}

inline const LayerElement& SubdomainElement::getLayerElement () const {
   return *m_layerElement;
}

inline bool SubdomainElement::isOnDomainBoundary ( const VolumeData::BoundaryId id ) const {
