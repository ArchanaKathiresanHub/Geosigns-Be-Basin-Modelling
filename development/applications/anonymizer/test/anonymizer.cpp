                        ...
                        double & originJ )
{
   ...
   // Read current value
   status = H5Dread( dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &originI);

   // Open an existing dataset
   dataset_id = H5Dopen2(fileId, "/origin in J dimension", H5P_DEFAULT);
   // Read current value
   status = H5Dread( dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &originJ);
...
TEST( AnonymizerTest, test_cbm00 )
{
   ...
   {
      // Check project file
      ibs::FilePath reference("test_cbm00");
      reference << "anonymized2compare" << anonymizer.getAnonymizedFileName();
      ibs::FilePath current_test("test_cbm00");
      current_test << anonymizer.s_anonymizedFolder << anonymizer.getAnonymizedFileName();
      ASSERT_TRUE(compareTextFiles(reference.fullPath().path(), current_test.fullPath().path()));
   }

   {
      // Check names mapping file
      ibs::FilePath reference("test_cbm00");
      reference << "anonymized2compare" << anonymizer.getNamesMappingFileName();
      ibs::FilePath current_test("test_cbm00");
      current_test << anonymizer.s_anonymizedFolder << anonymizer.getNamesMappingFileName();
      ASSERT_TRUE(compareTextFiles(reference.fullPath().path(), current_test.fullPath().path()));
   }
   ...
   for (const auto & mapElem : mapFiles)
   {
      ...
      ibs::FilePath current_test("test_cbm00");
      current_test << anonymizer.s_anonymizedFolder << mapElem.second;
      if (reference.fileNameExtension() == "HDF")
      {
         ASSERT_TRUE(compareHDFFiles(reference.fullPath().path(), current_test.fullPath().path()));
      }
      else
      {
         ASSERT_TRUE(compareTextFiles(reference.fullPath().path(), current_test.fullPath().path()));
