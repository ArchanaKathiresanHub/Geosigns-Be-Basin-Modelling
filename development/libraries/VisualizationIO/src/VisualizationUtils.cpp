...
std::shared_ptr<CauldronIO::SurfaceData> VisualizationUtils::cellCenterMap(const std::shared_ptr<CauldronIO::SurfaceData>& map, filterFunc filter, filterFunc filterUndef)
{
    ...
    for (int j = 0; j < numJ - 1; j++)
    {
        ...
        const float* inputRow2 = map->getRowValues(j + 1);
...
float VisualizationUtils::getUndefAverage(float* v, size_t size)
{
    ...
    float result = DefaultUndefinedValue;
    ...
    filterFunc filter, filterFunc filterUndef)
{
    ...
    for (size_t k = geometry->getFirstK(); k < geometry->getLastK(); k++)
    {
        ...
        const float* surface2 = volData->getSurface_IJ(k + 1);
        ...
        for (int j = 0; j < numJ - 1; j++)
        {
            ...
            const float* row12 = &surface1[numI*(j + 1)];
            const float* row21 = row22;
            const float* row22 = &surface2[numI*(j + 1)];
...
{
    filter = propertyIO->getName() == "Permeability" ? VisualizationUtils::getMedian : VisualizationUtils::getAverage;
    filterUndef = propertyIO->getName() == "Permeability" ? VisualizationUtils::getUndefMedian : VisualizationUtils::getUndefAverage;
}
