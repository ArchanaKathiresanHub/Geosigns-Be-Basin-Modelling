...
bool MasterTouch::run()
{
   ...
   if ( m_fileList.size() < 1 ) { return false; }
   ...
   for ( it = m_fileList.begin(); it != m_fileList.end(); ++it )
   {
      ...
      if ( Geocosm::TsLib::TSLIB_INTERFACE_VERSION != tslib->GetInterfaceVersion() ) 
      {
         //mismatch
      ...
      if ( tcfInfo->IsV2() )
      {
         try
         {
            ...
            if ( !m_directAnalogRun )
            {
               ...
               loaded = "Loaded TsLib 7.x Configuration file \"" + filename + "\"";
               setup  = "Setting up for a monte carlo calculation with " + ss.str() +" realizations"; 
            }
            else
            {
               loaded = "Loaded TsLib 7.x Configuration file \"" + filename + "\"";
         ...
         {  
            ExposeCOutCErr();
            cerr << endl << "MasterTouch::run()" << endl << ex << endl;
            return false;
         }
         catch ( std::exception & ex )
         {
            ExposeCOutCErr();
            cerr << endl << "MasterTouch::run()" << endl << ex.what() << endl;
            return false;
         }
      }
      else if ( tcfInfo->IsV1() )
      {
 ...
 */
bool MasterTouch::calculate( const char *filename, const Surface * surface, const Formation * formation )
{
 ...
 *  list which is saved to file at a later stage.
 */
bool MasterTouch::calculateOld( const char *filename, const Surface * surface, const Formation * formation )
{
   utilities::TimeToComplete timeToComplete( 30, 300, 0.1, 0.1 );
...
void MasterTouch::writeResultsToGrids( int i, int j, int timestepIndex )
{
   ...
   {   
      ...
      for ( int ii = 0; ii < numberOfTouchstoneProperties - 1; ++ii )
      {
         m_OutputProperties [  (ii + 1) * numberOfStatisticalOutputs - 1  ] = m_ActualOutputProperties [  ii ];
      }
   }
   else
   {
   ...
   {
      m_OutputProperties [ 6 * numberOfStatisticalOutputs + jj ] = log( m_OutputProperties [ 4 * numberOfStatisticalOutputs + jj ] );
   }
...
void MasterTouch::writeSummaryResults( GridMap *gridMap, short category, short format, int east, int north, int timestepIndex )
{
   ...
   // call TS summary 
   if ( TsLib_GetSummaryStatistics( timestepIndex, category, &summaryResults[MEAN], 
                                    &summaryResults[SD], &summaryResults[SKEWNESS],
                                    &summaryResults[KURTOSIS], &summaryMode, &nMode,
                                    &summaryResults[MIN], &summaryResults[MAX] 
                                  ) != 1 )
   {
...
void MasterTouch::writePercentileResults( GridMap * gridMap, short percent, short category, int east, int north, int timestepIndex )
{
   ...
   // call TS percentile
   if ( TsLib_GetPercentile( timestepIndex, category, percent, &percentileValue) != 1 )
   {
      touchstoneError ("writeResults");
      return;
   }
...
bool MasterTouch::retrieveGridMaps()
{
...
bool MasterTouch::restoreGridMaps()
{
   for (CategoryMapInfoList::iterator cmilIterator = m_currentOutputs.begin (); cmilIterator != m_currentOutputs.end (); ++cmilIterator)
   {
      MapInfoList & mapInfoList = (*cmilIterator).second;
...
void MasterTouch::touchstoneError( const char *function )
{
   ...
   cerr << endl << "MasterTouch::" << function << endl << errInfo->description << endl << errInfo->source << endl;
}
