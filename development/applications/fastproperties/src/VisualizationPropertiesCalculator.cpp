...
void VisualizationPropertiesCalculator::calculateProperties(FormationSurfaceVector& formationItems, Interface::PropertyList properties,
   ...
   for (snapshotIter = snapshots.begin(); snapshotIter != snapshots.end(); ++snapshotIter)
   {
      ...
      m_formInfoList = getDepthFormations(getProjectHandle(), snapshot);
...
void VisualizationPropertiesCalculator::writeToHDF() {
   ...
   CauldronIO::ExportToHDF::exportToHDF(vizProject, m_fileNameXml, 1, m_basement, getProjectHandle(), this);
}

//------------------------------------------------------------//
void VisualizationPropertiesCalculator::acquireAttributeProperties (DataModel::PropertyOutputAttribute outputAttribute) {

   Interface::PropertyListPtr simProperties = getProjectHandle().getProperties(outputAttribute);
...
void VisualizationPropertiesCalculator::acquireSimulatorProperties () {
   ...
   if (m_attribute != DataModel::UNKNOWN_PROPERTY_OUTPUT_ATTRIBUTE and m_propertyNames.empty())
   {
      ...
      {
         // check for Darcy
         const std::string& commLine = getProjectHandle().getDetailsOfLastSimulation ("fastcauldron")->getSimulatorCommandLineParameters();
...
{
   ImportProjectHandle import(false, m_vizProject, getProjectHandle());
   ...
   if (m_rank == 0)
   {
      if (m_attribute == DataModel::FASTMIG_PROPERTY or m_all2Dproperties)
      {
         ...
         database::Table* table = getProjectHandle().getTable("MigrationIoTbl");
         ...
         if (getProjectHandle().trappersAreAvailable())
         {
         ...
         if (getProjectHandle().trapsAreAvailable())
         {
...
std::shared_ptr<CauldronIO::Project> VisualizationPropertiesCalculator::createStructureFromProjectHandle(bool verbose) {
   ...
   snapShots.reset(getProjectHandle().getSnapshots(Interface::MAJOR | Interface::MINOR));
...
void VisualizationPropertiesCalculator::updateFormationsKRange() {
   ...
   const Snapshot * zeroSnapshot = getProjectHandle().findSnapshot(0);
   std::shared_ptr<CauldronIO::FormationInfoList> depthFormations = getDepthFormations(getProjectHandle(), zeroSnapshot);
...
void VisualizationPropertiesCalculator::createTrappers(const Interface::Snapshot * snapShot, std::shared_ptr<CauldronIO::SnapShot>& snapShotIO)
{
   ...
   {
      // Find trappers
      std::shared_ptr<Interface::TrapperList> trapperList(getProjectHandle().getTrappers(0, snapShot, 0, 0));
