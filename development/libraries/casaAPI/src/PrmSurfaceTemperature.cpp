    ...
    m_tableRow{0}
{
  try
  {
    ...
    const int rows = mdl.tableSize( tableName );
    bool ageAtSnapShot = true;
    double rowAge = 0;
    int row = rows-1;
    ...
    else
    {
      ...
      mdl.setTableValue(tableName, rows, ageColumnName, m_age);
      mdl.setTableValue(tableName, rows, temperatureColumnName, m_value);
