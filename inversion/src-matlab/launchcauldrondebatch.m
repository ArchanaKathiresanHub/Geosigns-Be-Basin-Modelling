...
for(r=1:Nruns)
    ...
    % heat cube model
    %LaunchLine = [Input.CauldronLauncher ' ' Input.Cluster ' ' num2str(Input.Processors) ' ' Input.CauldronVersion ' ' 'fastcauldron' ' ' '-project' ... 
     %   '  ' ActiveBatch(r).Name ' ' Input.CauldronOptions ' ' Input.CauldronMode ' ' '  > ' ActiveBatch(r).Name '_logfile 2>&1 </dev/null &'];
    
    
    LaunchLine = [Input.CauldronLauncher ' ' Input.Cluster ' ' num2str(Input.Processors) ' ' 'fastcauldron' ' ' '-project' ... 
        '  ' ActiveBatch(r).Name ' ' Input.CauldronOptions ' ' Input.CauldronMode ' ' '  > ' ActiveBatch(r).Name '_logfile 2>&1 </dev/null &'];
