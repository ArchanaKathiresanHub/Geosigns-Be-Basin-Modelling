...
{
   LogHandler( LogHandler::INFO ) << "Data digger requesting observables for base case...";
   ...
   LogHandler( LogHandler::INFO ) << "Adding base case project to the queue and generating scripts...";
   ...
   LogHandler( LogHandler::INFO ) << "Submitting jobs for base case to the cluster: " << rm.clusterName() << "...";
   ...
   {
      LogHandler( LogHandler::INFO ) << "Collecting observables values...";
      // collect observables value
      if ( ErrorHandler::NoError != sa->dataDigger().collectRunResults( sa->obsSpace(), sa->baseCaseRunCase() ) )
      {
         throw ErrorHandler::Exception( rm.errorCode() ) << rm.errorMessage();
      }
      LogHandler( LogHandler::INFO ) << "Base case execution succeeded";
   
      PrintObsValues( sa->baseCaseRunCase() );
   }
   else
   {
      LogHandler( LogHandler::INFO ) << "Base case execution failed";
   }
}
