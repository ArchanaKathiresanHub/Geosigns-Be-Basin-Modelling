...
TEST( DoubleExponentialPorosity, sub_arkose )
{
	...
	EXPECT_NEAR( 0.41 , myPorosity.porosity( 0.0e6, 0.0e6,false, 0.0 ), 1e-10);
	EXPECT_NEAR( 0.35784629093113574 , myPorosity.porosity( 5.0e6, 5.0e6,false, 0.0 ), 1e-10);
	EXPECT_NEAR( 0.12024352325 , myPorosity.porosity( 5.0e7, 5.0e7,false, 0.0 ), 1e-10);
	EXPECT_NEAR( 0.00005889633 , myPorosity.porosity( 5.0e8, 5.0e8,false, 0.0 ), 1e-10);
	
	//For different values of chemical compaction
	EXPECT_NEAR( 0.12024352325 , myPorosity.porosity( 5.0e7, 5.0e7,true, 0.0 ), 1e-10);
	EXPECT_NEAR( 0.03 , myPorosity.porosity( 5.0e7, 5.0e7, true, -1.0 ), 1e-10);
	...
	EXPECT_NEAR( 0.05 , myPorosity2.porosity( 5.0e10, 5.0e10,false, 0.0 ), 1e-10);
	//If there is both mechanical and chemical => chemical only is taken into account
	EXPECT_NEAR( 0.03 , myPorosity2.porosity( 5.0e7, 5.0e7,true, -1.0 ), 1e-10);
...
TEST( DoubleExponentialPorosity, extreme_coefficients_and_porosity )
{
	...
	EXPECT_NEAR( 0.03688972860 , typicalChalk.porosity( 50.0e6, 50.0e6,false, 0.0 ), 1e-10);
	...
	EXPECT_NEAR( 0.34825436772, ooidGraistone.porosity( 5.0e6, 5.0e6,false, 0.0 ), 1e-10);
	...
	EXPECT_NEAR( 0.0 , nulPorosity.porosity( 50.0e6, 50.0e6,false, 0.0 ), 1e-10);
	...
	EXPECT_NEAR( 0.70 , nulCoefficients.porosity( 50.0e6, 50.0e6,false, 0.0 ), 1e-10);
	...
	EXPECT_NEAR( 0.05 , extremeCoefficients.porosity( 50.0e6, 50.0e6,false, 0.0 ), 1e-10);
...
TEST( DoubleExponentialPorosity, elastic_rebound )
{
	...
	double withoutRebound = elasticRebound.porosity( 40.0e6, 40.0e6, false, 0.0 );
	double withoutRebound2 = elasticRebound.porosity( 50.0e6, 50.0e6, false, 0.0 );
	double withRebound = elasticRebound.porosity( 40.0e6, 50.0e6, false, 0.0 );
...
TEST( DoubleExponentialPorosity, single_is_special_case_double )
{
	...
	EXPECT_NEAR( doubleExp.porosity( 50.0e6, 50.0e6,false, 0.0 ), singleExp.porosity( 50.0e6, 50.0e6,false, 0.0 ), 1e-10);
	EXPECT_NEAR( doubleExp.porosity( 40.0e6, 50.0e6,false, 0.0 ), singleExp.porosity( 40.0e6, 50.0e6,false, 0.0 ), 1e-10);
}
