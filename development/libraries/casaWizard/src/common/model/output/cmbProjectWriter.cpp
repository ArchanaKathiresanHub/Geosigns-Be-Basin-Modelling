...
void CMBProjectWriter::addAllMapsToInputsFile()
{
   mbapi::MapsManager& mapsManager = cmbModel_->mapsManager();

   const std::vector<mbapi::MapsManager::MapID> mapIds = mapsManager.mapsIDs();
   for (mbapi::MapsManager::MapID mapId : mapIds)
   {
      if (cmbModel_->tableValueAsString("GridMapIoTbl", mapId, "MapFileName") != "Inputs.HDF")
      {
         const std::string mapName = cmbModel_->tableValueAsString("GridMapIoTbl", mapId, "MapName");
         const mbapi::MapsManager::MapID newId = mapsManager.copyMap(mapId, mapName, "Inputs.HDF");
...
void CMBProjectWriter::appendTimeStampToT2ZMaps(const QString &timeStamp)
{
   ...
   {
      const std::string mapName = cmbModel_->tableValueAsString("GridMapIoTbl", mapId, "MapName");
      const std::string referredTable = cmbModel_->tableValueAsString("GridMapIoTbl", mapId, "ReferredBy");
      mapsManager.removeMapReferenceFromGridMapIOTbl(mapName,referredTable);
   }
}
