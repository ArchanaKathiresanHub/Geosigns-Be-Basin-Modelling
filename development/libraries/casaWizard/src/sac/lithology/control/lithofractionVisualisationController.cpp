...
#include "view/assets/LithoGrid2DPlot.h"
...
{
   int counter = 0;
   for (const LithoGrid2DPlot* lithoFractionPlot : lithofractionVisualisation_->lithoFractionPlots())
   {
      connect(lithoFractionPlot->get2DView(), &LithoPercent2DView::toolTipCreated, [=](const QPointF& point){toolTipCreated(point, counter);});
      counter++;
   }
}
...
void LithofractionVisualisationController::slotUpdatePlots(const QString& layerName)
{
   ...
   int counter = 0;
   for (LithoGrid2DPlot* lithoPlot : lithofractionVisualisation_->lithoFractionPlots())
   {
      ...
      lithoPlot->get2DView()->updatePlots(values);
      lithoPlot->get2DView()->updateRange(xMin * Utilities::Maths::MeterToKilometer,
                                          xMax * Utilities::Maths::MeterToKilometer,
                                          yMin * Utilities::Maths::MeterToKilometer,
                                          yMax * Utilities::Maths::MeterToKilometer);
      lithoPlot->updateColorBar(2);
      lithoPlot->setTitle(lithologyTypes[counter], counter, !optimizedLithomapsAvailable);
      lithoPlot->get2DView()->setToolTipVisible(false);
      lithoPlot->get2DView()->setToolTipLithotypes(lithologyTypes);
...
std::vector<double> LithofractionVisualisationController::getLithopercentagesAtLocation(const QPointF& point) const
{
   ...
   {
      lithofractionsAtPoint.push_back(lithoFractionPlot->get2DView()->getValue(point));
   }
...
void LithofractionVisualisationController::toolTipCreated(const QPointF& point, const int plotID)
{
   ...
   if (!lithofractionsAtPoint.empty() && lithofractionIsValid(lithofractionsAtPoint[0]))
   {
      ...
      if (closestWellID != -1)
      {
         ...
         lithofractionVisualisation_->lithoFractionPlots()[plotID]->get2DView()->moveTooltipToDomainLocation(QPointF(closestWell.x() * Utilities::Maths::MeterToKilometer,
