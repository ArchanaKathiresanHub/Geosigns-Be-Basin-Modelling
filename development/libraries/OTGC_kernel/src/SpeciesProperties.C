...
void SpeciesProperties::Update()
{
   ...
   //IsMobile
   m_mobile = m_diffusionEnergy1 < OTGC::Constants::s_ActUMax ? true:false;
   //IsReactive
   m_reactive = m_activationEnergy1 < OTGC::Constants::s_ActUMax ? true:false;
   //Is HC
   ...
   if(AtomC > OTGC::Constants::s_CminHC && AtomH > OTGC::Constants::s_HminHC && AtomO < OmaxHC)
   {
      m_HC = true;
   }
   else
   ...
   if(m_diffusionEnergy1 < OTGC::Constants::s_ActUMax 
      && AtomC > OTGC::Constants::s_CminHC 
      && AtomH > OTGC::Constants::s_HminHC 
      && AtomO < OmaxHC 
      && m_density > OTGC::Constants::s_DensMaxGas)
   {
      m_Oil = true;
   }
   else
