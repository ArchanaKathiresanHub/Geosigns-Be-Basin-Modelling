...
void PressureSolver::setLayerElements ( const DA  femGrid,
   ...
   PETSC_3D_Array dofs ( femGrid, dofNumbers, INSERT_VALUES, true );
   ...
   while ( ! Layers.Iteration_Is_Done ()) {
      ...
      const LayerElementArray& volumeElements = layer->getLayerElements ();
            ...
            if ( mapElement.isValid ()) {
               ...
               for ( k = layer->getMaximumNumberOfElements () - 1; k >= 0; --k, --globalK ) {
                  ...
                  if ( mapElement.isOnProcessor ()) {
                     // get dof number at node positions.
                     n1 = int ( dofs ( globalK + 1, j,     i     ));
                     n2 = int ( dofs ( globalK + 1, j,     i + 1 ));
                     n3 = int ( dofs ( globalK + 1, j + 1, i + 1 ));
                     n4 = int ( dofs ( globalK + 1, j + 1, i     ));

                     n5 = int ( dofs ( globalK, j,     i     ));
                     n6 = int ( dofs ( globalK, j,     i + 1 ));
                     n7 = int ( dofs ( globalK, j + 1, i + 1 ));
                     n8 = int ( dofs ( globalK, j + 1, i     ));

                     // Segment is active if the dof numbers are different.
                     activeSegment1 = n1 != n5;
                     activeSegment2 = n2 != n6;
                     activeSegment3 = n3 != n7;
                     activeSegment4 = n4 != n8;
                     ...
                     if ( activeSegment1 or activeSegment2 or activeSegment3 or activeSegment4 ) {
                        ...
                        // Set node numbers.
                        volumeElement.setNodePosition ( 0, n1 );
                        volumeElement.setNodePosition ( 1, n2 );
                        volumeElement.setNodePosition ( 2, n3 );
                        volumeElement.setNodePosition ( 3, n4 );

                        volumeElement.setNodePosition ( 4, n5 );
                        volumeElement.setNodePosition ( 5, n6 );
                        volumeElement.setNodePosition ( 6, n7 );
                        volumeElement.setNodePosition ( 7, n8 );

                        // Determine if face is on domain boundary.
                        volumeElement.setIsOnDomainBoundary ( VolumeData::ShallowFace, n1 == topDof and n2 == topDof and n3 == topDof and n4 == topDof );
                        volumeElement.setIsOnDomainBoundary ( VolumeData::DeepFace, n5 == 0 and n6 == 0 and n7 == 0 and n8 == 0 );

                        volumeElement.setIsOnDomainBoundary ( VolumeData::Front, mapElements ( i, j ).isOnDomainBoundary ( MapElement::Front ));
                        volumeElement.setIsOnDomainBoundary ( VolumeData::Right, mapElements ( i, j ).isOnDomainBoundary ( MapElement::Right ));
                        volumeElement.setIsOnDomainBoundary ( VolumeData::Back,  mapElements ( i, j ).isOnDomainBoundary ( MapElement::Back ));
                        volumeElement.setIsOnDomainBoundary ( VolumeData::Left,  mapElements ( i, j ).isOnDomainBoundary ( MapElement::Left ));
               ...
               // for ( k = 0; k < layer->getMaximumNumberOfElements (); ++k, --globalK ) {
               for ( k = layer->getMaximumNumberOfElements () - 1; k >= 0; --k, --globalK ) {
                  LayerElement& volumeElement = layer->getLayerElement ( i, j, k );
      ...
      startGlobalK -= layer->getMaximumNumberOfElements ();
      Layers++;
   }
