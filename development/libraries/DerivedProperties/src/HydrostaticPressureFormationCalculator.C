...
void DerivedProperties::HydrostaticPressureFormationCalculator::computeHydrostaticPressureAtSeaBottomForHydrostatic ( const AbstractPropertyManager&     propertyManager,
   ...
   const DataAccess::Interface::RunParameters* runParameters = m_projectHandle.getRunParameters();
   const double temperatureGradient = 0.001 * runParameters->getTemperatureGradient ();
   if( fluid == 0 ) {
      fluidDensity = 0.0;
   } else {
