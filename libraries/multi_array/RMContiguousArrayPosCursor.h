#ifndef _MULTI_ARRAY_RMCONTIGUOUSARRAYPOSCURSOR_H_
#define _MULTI_ARRAY_RMCONTIGUOUSARRAYPOSCURSOR_H_

#include "multi_array/ContiguousSteppedCursor.h"

#define RMFORWARDPOSCURSOR RMContiguousArrayPosCursorBase
#define CURSOR ContiguousSteppedCursor<ENTRY>
#define CONST
...
class RMContiguousArrayPosCursor: 
  public RMContiguousArrayPosCursorBase<DIM,ENTRY>
{
public:
  ...
  inline RMContiguousArrayPosCursor(const CloseOpenInterval<DIM,int>& domain,
    const Tuple<DIM,int>& arraySteps, ENTRY* data):
    RMContiguousArrayPosCursorBase<DIM,ENTRY>( ContiguousSteppedCursor<ENTRY>( 
      domain.volume(), 1, data ), domain ) 
  {}
  ...
  inline RMContiguousArrayPosCursor(const CloseOpenInterval<DIM,int>& domain,
    ...
    RMContiguousArrayPosCursorBase<DIM,ENTRY>( ContiguousSteppedCursor<ENTRY>( 
      domain.volume(), 1, data, RMStrided2IndexMap<DIM>(domain.size()).
      index(domainCrd-domain.low()) ), domain, domainCrd )
  {}
};
