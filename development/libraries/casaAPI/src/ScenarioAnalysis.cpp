...
private:
   ...
   std::unique_ptr<VarSpaceImpl>              m_varSpace;           // influential parameters manager
...
void ScenarioAnalysis::ScenarioAnalysisImpl::restoreScenarioLocation( const char * pathToCaseSet )
{
   try
   {
      ...
      for ( size_t i = 0; i < m_doeCases->size(); ++i, ++m_caseNum )
      {
         ...
         if ( !errMsgs.empty() )
         {
            throw ErrorHandler::Exception( ErrorHandler::IoError ) << "Mismatched case influential parameters and completed case project: " <<
