#
#set -vx
# cfg2html_sun.sh v 1.13 27 june 2006
############################################################################
# Revision 1.13 27 june 2006 joachim:
#	Skip veritas, disksuite/svm when it is not used
# 	Added all inet files other than *ample to the network section
#	Created Solaris 10 'svcs -a' output and included legacy run scripts
#		(should behave like it used to on old versions)
#
# $Log: cfg2html_sun.sh,v $
# Revision 1.12  2005/05/19 13:43:41  ralproth
# + pack: rcs2log, fixed Log cvs keyword that contributors chnaged :)
#
# Revision 1.11  2005/05/19 13:30:08  ralproth
# SUN collector update by Keiran Sweet
#
# Revision 1.11 2004/08/23 10:33 ediaz
# new chage to prtdiag now use uname -m
# Test in primepower fujitsu, and solaris 9
# Revision 1.11B 2004/08/02 10:33 ediaz
# Some test in Solaris 9
# Change to exec prtdiag -v correctly
...
OUTDIR=`pwd`
#MVL
VERSION="Cfg2Html/SUN Version "$(echo "$Revision: 1.13 $" | cut -f2 -d" ")
#MVL
#
#
#
...
HTML_OUTFILE=$OUTDIR/`uname -n`_cfg.html
HTML_OUTFILE_TEMP=/tmp/`uname -n`.html.$$
TEXT_OUTFILE=$OUTDIR/`uname -n`_cfg.txt
TEXT_OUTFILE_TEMP=/tmp/`uname -n`.txt.$$
ERROR_LOG=$OUTDIR/`uname -n`_cfg.err
touch $HTML_OUTFILE
...
<HTML> <HEAD>
 ...
 <META NAME="DESCRIPTION" CONTENT="$Header: /home/CVS/cfg2html_sun/cfg2html_sun.sh,v 1.13 2006/06/27 Exp $DATE root Exp $">
...
close_html() {
   ...
   echo "</P><P>\n<hr><FONT COLOR=blue>Created "$DATEFULL" with " $VERSION " based upon v 1.12 by <A HREF="mailto:t505414@online.no?subject=$VERSION_">Trond E. Aune, SysAdm</A></P></font>" >> $HTML_OUTFILE_TEMP
...
if [ "$CFG_HARDWARE" != "no" ] ; then
   ...
   exec_command "/usr/platform/`uname -m`/sbin/prtdiag -v" "Hardware (prtdiag)"
...
if [ "$CFG_FILESYS" != "no" ] ; then
   ...
   exec_command "df -hl" "Local filesystems and usage"
...
###########################################################################
  ...
  if [ -e "/usr/sbin/metadb" ] ; then
	  pathsds="/usr/sbin"
				 ...
				 # use correct path for metadb:
if [ "$CFG_DISKSUITE" != "no" -a -x ${pathsds}/metadb -a \
	`${pathsds}/metadb >/dev/null 2>&1` -eq 0 ] ; then
...
if [ "$CFG_NETWORK" != "no" ] ; then
   ...
   COUNT=1
   for FILE in `ls /etc/resolv.conf /etc/hostn* /etc/nodename /etc/inet/*|\
		grep -v ample`
   do
      if [ -s ${FILE} ]; then
...
if [ "$CFG_CRON" != "no" ] ; then
   ...
   exec_command crontab_collect "Crontab and AT scheduler"
...
if [ "$CFG_FILES" != "no" ] ; then
   ...
   paragraph "Boot and/or Service configuration"
   inc_heading_level
   exec_command "cat /etc/inittab" "inittab"
  if [ `uname -r | cut -d. -f2` -ge 10 -a \
	`uname -r | cut -d. -f1` -ge 5 ]; then
      exec_command "svcs -a | tee /tmp/cfg2htmlsvc.$$" "svcs -a"
ls /tmp/cfg2htmlsvc.$$
	FILES=`sed -n 's/^legacy_run.*://;s/_d\//\.d\//;/etc/p' \
		/tmp/cfg2htmlsvc.$$`
	rm -f /tmp/cfg2htmlsvc.$$
echo $FILES
  else
   files()
   {
      #ls /etc/resolv.conf /etc/hostn* /etc/nodename /etc/inet/*|grep -v ample
      ls /etc/rc2.d/*
      ls /etc/rc3.d/*
   }
	FILES=`files`
...
#-------------------------
   ...
   ### VXFS ##################################################################
   if [ "$CFG_VXFS" != "no" -a `pkginfo VRTSvxfs >/dev/null 2>&1` -eq 0 ] ; then
