...
void Basin_Modelling::Fundamental_Property_Manager::Set_Layer_DA ( const DA* Layer_Array ) {
  //
  //
...
void Basin_Modelling::Fundamental_Property_Manager::Create_Properties () {
  ...
  for ( Property = Depth; Property < No_Property; Property++ ) {
    ...
    if ( Vector_Properties [ Property ] != Vec_Ptr ( 0 )) {
      Error = DACreateGlobalVector ( *Layer_DA, Vector_Properties [ Property ]);
    } 
...
void Basin_Modelling::Fundamental_Property_Manager::Copy ( const Fundamental_Property_Manager& Manager,
  ...
  DAGetCorners ( *Layer_DA, &X_Start, &Y_Start, &Z_Start, &X_Count, &Y_Count, &Z_Count );
...
void Basin_Modelling::computeBulkDensityVectors ( AppCtx* Basin_Model ) {
  ...
  DAGetCorners ( *Basin_Model->mapDA, &xStart, &yStart, PETSC_NULL, &xCount, &yCount, PETSC_NULL );
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> BulkDensity );
...
void Basin_Modelling::computeVelocityVectors ( AppCtx*        Basin_Model ) {
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> Velocity );
...
//    Layers.Initialise_Iterator ( Basin_Model->layers, Descending, Basement_And_Sediments, Active_Layers_Only );
  ...
  while ( ! Layers.Iteration_Is_Done () ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> Reflectivity );
...
void Basin_Modelling::computeSonicVectors ( AppCtx*        Basin_Model ) {
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> Sonic );
...
void Basin_Modelling::computeThermalConductivityVectors ( AppCtx*        Basin_Model ) {
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> BulkTHCondN );
    VecSet ( currentLayer -> BulkTHCondN, CAULDRONIBSNULLVALUE );

    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> BulkTHCondP );
...
void Basin_Modelling::computeDiffusivityVectors ( AppCtx*        Basin_Model ) {
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> Diffusivity );
...
void Basin_Modelling::computeThicknessVectors ( AppCtx*        Basin_Model ) {
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    //
    DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> layerThickness );
...
//    }
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    // Output of this vector depends on the layerDA which has the full 3D dimension.
    //
//      DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> PermeabilityV );
//      DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> PermeabilityH );
...
#undef  __FUNCT__  
#define __FUNCT__ "Basin_Modelling::deletePermeabilityVectors"

//  void Basin_Modelling::deletePermeabilityVectors ( AppCtx* Basin_Model ) {
...
//    for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
//      VecValid ( Layers.Current_Layer () -> layerPermeability, &validVector );

//      //
//      // The vector should be valid. some other comment
//      //
//      if ( validVector ) {
//        VecDestroy ( Layers.Current_Layer () -> layerPermeability );
//        Layers.Current_Layer () -> layerPermeability = Vec ( 0 );
//      }

//    }

//  } // end Basin_Modelling::deletePermeabilityVectors


//------------------------------------------------------------//

#undef  __FUNCT__  
#define __FUNCT__ "Basin_Modelling::computePorosityVectors"

void Basin_Modelling::computePorosityVectors ( AppCtx*        Basin_Model ) {
...
//    }
  ...
  for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
    ...
    VecSet ( currentLayer -> Porosity, CAULDRONIBSNULLVALUE );
      ...
      for ( J = yStart; J < yStart + yCount; J++ ) {
         ...
         if ( Basin_Model->nodeIsDefined ( I, J )) {
          ...
          for ( K = zStart; K < zStart + zCount; K++ ) {
            ...
            if ( Basin_Model->Do_Iteratively_Coupled || Basin_Model->IsCalculationCoupled || Basin_Model->DoOverPressure ) {
              layerPorosity ( K, J, I ) = currentLithology->porosity ( VES, maxVES, includeChemicalCompactionTerm, layerChemicalCompaction ( K, J, I ));
            } else {
              layerPorosity ( K, J, I ) = currentLithology->porosity ( maxVES, maxVES, false, 0.0 );
            }
...
//  void Basin_Modelling::computeFaultElementMaps ( AppCtx*        Basin_Model,
...
//    DAGetCorners ( *basinModel->mapDA, &xStart, &yStart, PETSC_NULL, &xCount, &yCount, PETSC_NULL );

//    for ( Layers.Initialise_Iterator (); ! Layers.Iteration_Is_Done (); Layers++ ) {
//      currentLayer = Layers.Current_Layer ();

//      PETSC_3D_Array layerFaultElements;

//      DAGetCorners ( currentLayer->layerDA, PETSC_NULL, PETSC_NULL, &zStart, PETSC_NULL, PETSC_NULL, &zCount );
//      DACreateGlobalVector ( currentLayer -> layerDA, &currentLayer -> faultElements );
//      VecSet ( &CAULDRONIBSNULLVALUE, currentLayer -> faultElements );

//      layerFaultElements.Set_Global_Array ( currentLayer -> layerDA, 
//                                            currentLayer -> faultElements );
...
//            currentLithology = currentLayer -> Lithology ( I, J );

//            if ( currentLithology->isFault ()) {
//              Value = 1.0;
//            } else {
//              Value = 0.0;
//            }
...
//            for ( K = zStart; K < zStart + zCount; K++ ) {
//              layerFaultElements ( K, J, I ) = Value;
//  	  }
...
void Basin_Modelling::computeBasementLithostaticPressureForCurrentTimeStep ( AppCtx* basinModel,
   ...
   if ( basinModel->isALC() ) {
      ...
      if ( age <= bottomLayer->depoage ) {
         ...
         DAGetCorners ( bottomLayer->layerDA,
      ...
      for ( layer = basinModel->layers.size () - 2; layer < basinModel->layers.size (); ++layer ) {
         ...
         if(( currentLayer -> isCrust()  && FastcauldronSimulator::getInstance ().getCrustPropertyModel()  != "Legacy Crust" ) ||
            ...
            DAGetCorners ( currentLayer->layerDA,
