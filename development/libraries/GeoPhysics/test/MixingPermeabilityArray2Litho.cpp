...
TEST(MixingPermeabilityArray, TwoLithoHomogeneousNoAnisoSamePerm)
{


   std::vector<double> anisoVec ( { 1.0, 1.0 });
   std::vector<double> percentVec ( { 50.0, 50.0 } );
...
TEST(MixingPermeabilityArray, TwoLithoHomogeneousAniso )
{


   std::vector<double> anisoVec ( { 1.0, 2.0 });
   std::vector<double> percentVec ( { 50.0, 50.0 } );
...
TEST(MixingPermeabilityArray, TwoLithoLayeredAniso )
{
   ...
   for ( int t = 0; t < NumberOfLayeringIndices; ++t ) {
      ...
      for ( int i = 0; i < NumberOfValues; ++i ) {
         expectedPermeabilityNormal [ i ] = std::pow ( fracVec [ 0 ] * std::pow ( simplePermeabilities.getData ( 0 )[ i ], verticalPower ) +
                                                       fracVec [ 1 ] * std::pow ( simplePermeabilities.getData ( 1 )[ i ], verticalPower ), 1.0 / verticalPower );

         expectedPermeabilityPlane [ i ] = std::pow ( fracVec [ 0 ] * std::pow ( anisoVec [ 0 ] * simplePermeabilities.getData ( 0 )[ i ], horizontalPower ) +
                                                      fracVec [ 1 ] * std::pow ( anisoVec [ 1 ] * simplePermeabilities.getData ( 1 )[ i ], horizontalPower ), 1.0 / horizontalPower );
      }
...
TEST(MixingPermeabilityArray, TwoLithoLayeredAniso_LI_0_25 )
{
...
TEST(MixingPermeabilityArray, TwoLithoLayeredAniso_LI_0 )
{


   std::vector<double> anisoVec ( { 1.0, 2.0 });
   std::vector<double> percentVec ( { 30.0, 70.0 } );
   std::vector<double> fracVec;
