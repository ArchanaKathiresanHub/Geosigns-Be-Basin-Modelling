...
#define USE_AVERAGED_PERMEABILITIES 1
...
void ExplicitMultiComponentFlowSolver::solve ( Subdomain&   subdomain,
   ...
   double deltaTSec;
   double subTimeStepStart;
   double subTimeStepEnd;
   double lambdaStart;
   double lambdaEnd;
   ...
   sourceTerm.create ( concentrationGrid );
   previousTerm.create ( concentrationGrid );
   ...
   if ( subdomain.sourceRockIsActive () and FastcauldronSimulator::getInstance ().getMcfHandler ().useCflTimeStepping ()) {
      ...
      double cflValue = FastcauldronSimulator::getInstance ().getMcfHandler ().cflTimeStepFraction () * computeCflNumber ( subdomain ) / GeoPhysics::SecondsPerMillionYears;

      numberOfSubTimeSteps = int ( std::ceil (( startTime - endTime ) / cflValue ));
      numberOfSubTimeSteps = NumericFunctions::Maximum ( 4, numberOfSubTimeSteps );
      fractionScaling = 1.0 / double ( numberOfSubTimeSteps );
      deltaTMa = ( startTime - endTime ) * fractionScaling;
      deltaTSec = GeoPhysics::SecondsPerMillionYears * deltaTMa;
   } else if ( subdomain.sourceRockIsActive () and ( startTime - endTime ) > FastcauldronSimulator::getInstance ().getMcfHandler ().getMaximumTimeStepSize ()) {
      numberOfSubTimeSteps = int ( std::ceil (( startTime - endTime ) / FastcauldronSimulator::getInstance ().getMcfHandler ().getMaximumTimeStepSize ()));
      fractionScaling = 1.0 / double ( numberOfSubTimeSteps );
      deltaTMa = ( startTime - endTime ) * fractionScaling;
      deltaTSec = GeoPhysics::SecondsPerMillionYears * deltaTMa;
   } else {
      numberOfSubTimeSteps = 1;

      deltaTMa = ( startTime - endTime );
      deltaTSec = GeoPhysics::SecondsPerMillionYears * deltaTMa;

      fractionScaling = 1.0;
   }

   if ( FastcauldronSimulator::getInstance ().getMcfHandler ().getDebugLevel () > 0 and
        FastcauldronSimulator::getInstance ().getRank () == 0 ) {
      cout << " MCF time step info: "
           << numberOfSubTimeSteps 
           << " start = " << startTime
           << "Ma, end = " << endTime
           << "Ma, delta-t = " << deltaTMa
           << "Ma, delta-t = " << deltaTSec
           << " sec, fraction = " << fractionScaling
           << "." << endl;
   }
   ...
   double subSampledSubTimeStepStart;
   double subSampledSubTimeStepEnd;
   double subSampledLambdaStart;
   ...
   subTimeStepStart = startTime;
   subTimeStepEnd   = startTime - deltaTMa;

   lambdaStart = 0.0;
   lambdaEnd = fractionScaling;

   subSampledSubTimeStepStart = subTimeStepStart;
   subSampledSubTimeStepEnd =  subTimeStepEnd;

   subSampledLambdaStart = lambdaStart;
   subSampledLambdaEnd = lambdaEnd;

   for ( t = 0; t < numberOfSubTimeSteps; ++t ) {
      iterationStart = WallTime::clock ();

      if ( m_timeStepSubSample > 1 and t % m_timeStepSubSample == 0 ) {
         timeStepSubSamplePoint = true;

         if ( t + numberOfSubTimeSteps % m_timeStepSubSample >= numberOfSubTimeSteps ) {
            scaling = numberOfSubTimeSteps % m_timeStepSubSample;
         } else {
            scaling = m_timeStepSubSample;
         }

      } else {
         timeStepSubSamplePoint = false;
      }
      ...
      if ( not m_timeStepSubSampleOtgc or ( m_timeStepSubSampleOtgc and timeStepSubSamplePoint )) {
         // Apply OTGC to (previous-) components for all elements in the subdomain.
         otgcStart = WallTime::clock ();
         applyOtgc ( subdomain, subSampledSubTimeStepStart, subSampledSubTimeStepEnd, subSampledLambdaStart, subSampledLambdaEnd );
         otgcIntervalTime += WallTime::clock () - otgcStart;
      }

      if ( not m_timeStepSubSamplePvt or ( m_timeStepSubSamplePvt and timeStepSubSamplePoint )) {
      ...
      if ( not m_timeStepSubSampleFlux or ( m_timeStepSubSampleFlux and timeStepSubSamplePoint )) {
         ...
         // The sub-sampled lambda-end is not used here because ...
         // The delta-T remains the same because it should represent the flux over the delta-T time-interval.
      ...
      if ( not stopHcTransport or subTimeStepEnd >= stopHcTransportAge ) {
      ...
      if ( not removeSourceTerm or subTimeStepEnd >= sourceTermRemovalAge ) {
         // integrate over the element the hc generated in genex.
         sourceStart = WallTime::clock ();
         computeSourceTerm ( subdomain, computedConcentrations, fractionScaling );
      ...
      subTimeStepStart = subTimeStepEnd;
      subTimeStepEnd -= deltaTMa;

      lambdaStart = lambdaEnd;
      lambdaEnd += fractionScaling;

      if ( timeStepSubSamplePoint ) {
         subSampledSubTimeStepStart = subSampledSubTimeStepEnd;
         subSampledSubTimeStepEnd -=  scaling * deltaTMa;

         subSampledLambdaStart = subSampledLambdaEnd;
         subSampledLambdaEnd += scaling * fractionScaling;
      } else if ( m_timeStepSubSample == 1 ) {
         subSampledSubTimeStepStart = subTimeStepStart;
         subSampledSubTimeStepEnd =  subTimeStepEnd;
...
void ExplicitMultiComponentFlowSolver::transportComponents ( FormationSubdomainElementGrid&   formationGrid,
   ...
   FiniteElementMethod::ElementVector phasePressure;
   ...
   PVTComponents  transportedComponents;
   PVTComponents  elementGasComponents;
   PVTComponents  elementOilComponents;

   PVTPhaseValues phases;
   PVTPhaseComponents phaseComponents;
   Saturation     saturations;

   PVTComponents  gasExported; 
   PVTComponents  oilExported; 
   PVTComponents  gasImported; 
   PVTComponents  oilImported; 

   double elementVolume;

   double elementGasComponentSum;
   double elementOilComponentSum;
   ...
   for ( i = elementGrid.firstI (); i <= elementGrid.lastI (); ++i ) {
      ...
      for ( j = elementGrid.firstJ (); j <= elementGrid.lastJ (); ++j ) {
         ...
         for ( k = formationGrid.lastK (); k >= formationGrid.firstK (); --k ) {
            ...
            const ElementFaceValues& elementGasFlux = gasFluxes ( elementK, j, i );
            const ElementFaceValues& elementOilFlux = oilFluxes ( elementK, j, i );

            // mol/m^3
            elementGasComponents = phaseComposition ( i, j, elementK ).getPhaseComponents ( pvtFlash::VAPOUR_PHASE );
            elementOilComponents = phaseComposition ( i, j, elementK ).getPhaseComponents ( pvtFlash::LIQUID_PHASE );

            elementGasComponentSum = elementGasComponents.sum ();
            elementOilComponentSum = elementOilComponents.sum ();

            if ( layerElement.isActive ()) {
               double theFlux;
               double theNeighbourFlux;

               elementVolume = volumeOfElement ( layerElement );
      ...
      for ( j = elementGrid.firstJ (); j <= elementGrid.lastJ (); ++j ) {
         ...
         for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
            ...
            if ( layerElement.isActive ()) {
               ...
               getGeometryMatrix ( element.getLayerElement (), geometryMatrix );
               finiteElement.setGeometry ( geometryMatrix );

               elementVolume = poreVolumeOfElement ( element.getLayerElement (), geometryMatrix );
               ...
               concentrationSum = composition.sum ();

               // Need to multiply by ratio of element-volume / element-pore-volume
               if ( phases ( pvtFlash::VAPOUR_PHASE ) > ConcentrationLowerLimit ) {
                  vapourSaturation = concentrationSum * vapourFraction * vapourMolarMass / phaseDensities ( i, j, elementK )( pvtFlash::VAPOUR_PHASE );
               } else {
                  vapourSaturation = 0.0;
               }

               if ( phases ( pvtFlash::LIQUID_PHASE ) > ConcentrationLowerLimit ) {
                  liquidSaturation = concentrationSum * ( 1.0 - vapourFraction ) * liquidMolarMass / phaseDensities ( i, j, elementK )( pvtFlash::LIQUID_PHASE );
               } else {
               ...
               // Should include immobile-species here.
               ...
               sat.set ( liquidSaturation, vapourSaturation );
#endif

               sat = saturations ( element.getI (), element.getJ (), element.getK ());

               const int NumberOfFluxFaces = 6;
               VolumeData::BoundaryId fluxFaces [ NumberOfFluxFaces ] = { VolumeData::GAMMA_2, VolumeData::GAMMA_3, VolumeData::GAMMA_4,
               ...
               if ( phases ( pvtFlash::LIQUID_PHASE ) > ConcentrationLowerLimit ) {
                  ...
                  for ( face = 0; face < NumberOfFluxFaces; ++face ) {
                     ...
                     if ( element.getLayerElement ().isActiveBoundary ( id )) {
                        ...
                        elementPressure = subdomainLiquidPressure ( element.getK (), element.getJ (), element.getI ());

                        if ( neighbour != 0 ) {
                           neighbourPressure = subdomainLiquidPressure ( neighbour->getK (), neighbour->getJ (), neighbour->getI ());
                        } else {
                           // What to put here!
                        ...
                        v3 = computeElementFaceFlux ( element, id, Saturation::OIL, 
                                                      elementPressure,
                                                      neighbourPressure,
                                                      deltaX,
                                                      phaseDensities ( i, j, elementK )( pvtFlash::LIQUID_PHASE ),
                                                      phaseViscosities ( i, j, elementK )( pvtFlash::LIQUID_PHASE ),
                                                      sat,
                                                      finiteElement,
                        ...
                        elementOilFlux ( id ) = deltaT * oilMassDensity * v3 / liquidMolarMass;
                     } else {
                        elementOilFlux ( id ) = 0.0;
                     }

                     if ( OUTPUT ) {
                        temporary = phaseComponents.getPhaseComponents ( pvtFlash::LIQUID_PHASE );
                        temporary *= elementOilFlux ( id );

                        cout << " oil flux of element  " << i << "  " << j << "  " << elementK << "  "
                             << VolumeData::boundaryIdImage ( id ) << " =  " << elementOilFlux ( id ) << "  "
                             << deltaT << "  " << oilMassDensity << "  " << v3 << "  " << liquidMolarMass
                             << std::endl
                             << temporary.image () << std::endl;
                        temporary *= 1.0 / elementVolume;
                        cout << temporary.image () << std::endl;
                     }

                  }

#if 0
                  double oilFlux = elementOilFlux.sumGt0 ();

                  if ( oilFlux > 0.0 and oilFlux > m_maximumHCFractionForFlux * totalHc ( pvtFlash::LIQUID_PHASE )) {
                     elementOilFlux *= m_maximumHCFractionForFlux * totalHc ( pvtFlash::LIQUID_PHASE ) / oilFlux;
                  }
#endif
               ...
               if  ( phases ( pvtFlash::VAPOUR_PHASE ) > ConcentrationLowerLimit ) {
                  ...
                  //    const VolumeData::BoundaryId id = static_cast<VolumeData::BoundaryId>( face );
                     ...
                     if ( element.getLayerElement ().isActiveBoundary ( id )) {
                        ...
                        elementPressure = subdomainVapourPressure ( element.getK (), element.getJ (), element.getI ());

                        if ( neighbour != 0 ) {
                           neighbourPressure = subdomainVapourPressure ( neighbour->getK (), neighbour->getJ (), neighbour->getI ());
                        } else {
                           // What to put here!
                           // neighbourPressure = NumericFunctions::Maximum ( 0.0, 1.0e6 * porePressure ( element, lambda ) - 1.0e6 );
                        ...
                        v3 = computeElementFaceFlux ( element, id, Saturation::GAS, 
                                                      elementPressure,
                                                      neighbourPressure,
                                                      deltaX,
                                                      phaseDensities ( i, j, elementK )( pvtFlash::VAPOUR_PHASE ),
                                                      phaseViscosities ( i, j, elementK )( pvtFlash::VAPOUR_PHASE ),
                                                      sat,
                                                      // vapourSaturation,
                                                      finiteElement,
                                                      elementPermeabilityN ( id ),
                        ...
                        elementGasFlux ( id ) = deltaT * gasMassDensity * v3 / vapourMolarMass;
                     } else {
                        elementGasFlux ( id ) = 0.0;
                     }

                     if ( OUTPUT ) {

                        temporary = phaseComponents.getPhaseComponents ( pvtFlash::VAPOUR_PHASE );
                        temporary *= elementGasFlux ( id );

                        cout << " gas flux of element " << i << " " << j << " " << elementK << " "
                             << VolumeData::boundaryIdImage ( id ) << " =  " << elementGasFlux ( id ) << "  "
                             << deltaT << "  " << gasMassDensity << "  " << v3 << "  " << vapourMolarMass
                             << std::endl
                             << temporary.image () << std::endl;
                        temporary *= 1.0 / elementVolume;
                        cout << temporary.image () << std::endl;
                        cout << " total mass per element " << std::endl << t1.image () << std::endl;
                     }

                  }

#if 0
                  double gasFlux = elementGasFlux.sumGt0 ();

                  if ( gasFlux > 0.0 and gasFlux > m_maximumHCFractionForFlux * totalHc ( pvtFlash::VAPOUR_PHASE )) {
                    elementGasFlux *= m_maximumHCFractionForFlux * totalHc ( pvtFlash::VAPOUR_PHASE ) / gasFlux;
                  }
#endif
               ...
               double oilFlux = elementOilFlux.sumGt0 ();
               double gasFlux = elementGasFlux.sumGt0 ();
               double fluxSum = gasFlux + oilFlux;
               double hcSum = totalHc.sum ();

               if ( hcSum > 0.0 and fluxSum > m_maximumHCFractionForFlux * hcSum ) {
                  elementGasFlux *= m_maximumHCFractionForFlux * totalHc ( pvtFlash::VAPOUR_PHASE ) / fluxSum;
                  elementOilFlux *= m_maximumHCFractionForFlux * totalHc ( pvtFlash::LIQUID_PHASE ) / fluxSum;
               }
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               if ( element.getLayerElement ().isActive ()) {
                  ...
                  if ( elementConcentrations.sum () > ConcentrationLowerLimit ) {
                     ...
                     // // Convert to kelvin.
                     // temperature = interpolateProperty ( element.getLayerElement (), Basin_Modelling::Temperature, lambda ) + 273.15;

                     // // Convert to pascals from mega-pascals.
                     // porePressure = NumericFunctions::Maximum ( 1.0e6 * interpolateProperty ( element.getLayerElement (), Basin_Modelling::Pore_Pressure, lambda ), 1.0e5 );

                     // cout << " flash : " << element.getI () << "  " << element.getJ ()  << "  " << element.getK ()  << "  " 
                     //      << temperature ( element, lambda ) << "  "
                     //      << porePressure ( element, lambda )
                     //      << endl;
                     ...
                     if ( OUTPUT ) {
                        ...
                        elementVolumeCalculations ( element.getLayerElement (), elementVolume, poreVolume );

                        gasMass = phaseComposition ( i, j, elementK ).getPhaseComponents ( pvtFlash::VAPOUR_PHASE ).sum () * elementVolume;
                        gasVolume = gasMass / phaseDensities ( i, j, elementK )( pvtFlash::VAPOUR_PHASE );
                        oilMass = phaseComposition ( i, j, elementK ).getPhaseComponents ( pvtFlash::LIQUID_PHASE ).sum () * elementVolume;
                        oilVolume = oilMass / phaseDensities ( i, j, elementK )( pvtFlash::LIQUID_PHASE );

                        cout << " element " << element.getI () << "  " << element.getJ () << "  " << element.getK () << "  " << std::endl;
                        cout << " hc volumes " 
                             << elementVolume << "  " << poreVolume << "  " 
                             << gasMass << "   " << gasVolume << "  " 
                             << oilMass << "   " << oilVolume << "  " << std::endl;
                        cout << " hc densities  : " << phaseDensities ( i, j, elementK ).image () << std::endl
                             << " hc viscosities: " << phaseViscosities ( i, j, elementK ).image () << std::endl;
                     }
                  ...
                  if ( OUTPUT ) {
                     // cout << " conditions: " << porePressure << "  " << temperature << "  " << ( elementGrid.isPartOfStencil ( i, j ) ? "true" : "false" ) << std::endl;
                  
                     cout << " molar masses       : " << molarMasses.image () << std::endl;
                     cout << " concentrations     : " << concentrations ( k, j, i ).image () << std::endl;
                     cout << " unit masses        : " << (concentrations ( k, j, i ) * molarMasses ).image () << std::endl;
                     cout << " element components : " << elementConcentrations.image () << std::endl;
                  
                     cout << " flashed            : " << std::endl 
                          << phaseComposition ( i, j, k ).image () << std::endl;
                  
                     cout << " props   : " << std::endl 
                          << phaseDensities ( i, j, elementK ).image () << std::endl
                          << phaseViscosities ( i, j, elementK ).image () << std::endl;
                  }
...
void ExplicitMultiComponentFlowSolver::flashComponents ( Subdomain&             subdomain,
   ...
   while ( not iter.isDone ()) {
      flashComponents ( *iter, iter->getVolumeGrid ( NumberOfPVTComponents ),
                        phaseComposition, phaseDensities, phaseViscosities,
...
void ExplicitMultiComponentFlowSolver::computeSourceTerm ( const SubdomainElement& element,
   ...
   if ( element.getLayerElement ().isActive ()) {
      ...
      if ( print ) {
         ...
         gen = computedSourceTerm;
         gen *= 1.0 / elementVolume;

         div = generated ( 1 );
         div /= gen;

         cout << " Source term: element volume " << elementVolume << std::endl
              << " generated  : " << computedSourceTerm.image () << std::endl
              << " genex      : " << generated ( 1 ).image () << std::endl
              << " gen conc   : " << gen.image () << std::endl
              << " div        : " << div.image () << std::endl << endl;
      }

      // After integration: kg/element/time-step.
      ...
      for ( j = concentrationGrid.firstJ (); j <= concentrationGrid.lastJ (); ++j ) {
         ...
         if ( FastcauldronSimulator::getInstance ().getMapElement ( i, j ).isValid ()) {
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               computeSourceTerm ( element, sourceTerm ( i, j, element.getK ()), fractionScaling, OUTPUT );
...
void ExplicitMultiComponentFlowSolver::computeSourceTerm ( Subdomain&        subdomain,
   ...
   while ( not iter.isDone ()) {
      computeSourceTerm ( *iter, iter->getVolumeGrid ( NumberOfPVTComponents ), fractionScaling, sourceTerm );
      ++iter;
   }
...
double ExplicitMultiComponentFlowSolver::computeElementMassMatrix ( const SubdomainElement& element,
   ...
   // NumericFunctions::Quadrature3D::getInstance ().get ( m_massMatrixQuadratureDegree, quad );
         ...
         if ( FastcauldronSimulator::getInstance ().getMapElement ( i, j ).isValid ()) {
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               if ( element.getLayerElement ().isActive ()) {
                  ...
                  if ( OUTPUT ) {
                     sum1 += computedConcentrations ( i, j, elementK ) * volumeOfElement ( element.getLayerElement ());
                     cout << " concentrations: " << std::setw ( 2 ) << i << "  " << std::setw ( 2 ) << j << "  " << std::setw ( 2 ) << elementK << "  " << endl
                          << concentrations ( k, j, i ).image () << endl
                          << elementConcentration.image ()
                          << std::endl
                          // << " unit masses              " << (computedConcentrations ( i, j, elementK )).image () << std::endl
                          // << " masses                   " << (computedConcentrations ( i, j, elementK ) * volumeOfElement ( element.getLayerElement ())).image ()
                          << std::endl
                          << std::endl;
                  }
...
void ExplicitMultiComponentFlowSolver::computeSaturation ( const SubdomainElement&        element,
   ...
   if ( print ) {
      cout << " full saturations: " << saturation.image () << "  " << hcSaturation << std::endl
           << " saturation calculation gas : "
           << std::setw ( 9 ) << concentrationSum << "  " 
           << std::setw ( 9 ) << vapourFraction << "  "
           << std::setw ( 9 ) << vapourMolarMass << "  "
           << std::setw ( 9 ) << density ( pvtFlash::VAPOUR_PHASE ) << " = "
           << std::setw ( 9 ) << saturation ( Saturation::VAPOUR ) << std::endl
           << " saturation calculation oil : "
           << std::setw ( 9 ) << concentrationSum << "  " 
           << std::setw ( 9 ) << 1.0 - vapourFraction << "  " 
           << std::setw ( 9 ) << liquidMolarMass << "  "
           << std::setw ( 9 ) << density ( pvtFlash::LIQUID_PHASE ) << " = "
           << std::setw ( 9 ) << saturation ( Saturation::LIQUID ) << std::endl
           << " Immobile Saturations " << immobileSaturation << endl;
   }

#if 0
   if ( isnan ( saturation ( Saturation::WATER  )) or isinf ( saturation ( Saturation::WATER  )) or saturation ( Saturation::WATER  ) < 0.0 or saturation ( Saturation::WATER  ) > 1.0 or
        isnan ( saturation ( Saturation::VAPOUR )) or isinf ( saturation ( Saturation::VAPOUR )) or saturation ( Saturation::VAPOUR ) < 0.0 or saturation ( Saturation::VAPOUR ) > 1.0 or
        isnan ( saturation ( Saturation::LIQUID )) or isinf ( saturation ( Saturation::LIQUID )) or saturation ( Saturation::LIQUID ) < 0.0 or saturation ( Saturation::LIQUID ) > 1.0 ) {
      cout << " incorrect final saturation: " << element.getI ()  << "  " << element.getJ ()  << "  " << element.getK ()  << "  " << endl << flush;
      cout << density.image () << endl << flush;
      cout << phaseMasses.image () << endl << flush;
   }
#endif
...
double ExplicitMultiComponentFlowSolver::computeCflNumber ( const FormationSubdomainElementGrid& formationGrid ) {

   const LayerProps& theLayer = formationGrid.getFormation ();

   FiniteElementMethod::ElementGeometryMatrix geometryMatrix;
   FiniteElementMethod::ElementVector temperatureCoeffs;
   FiniteElementMethod::ElementVector phasePressureCoeffs;
   ...
   PVTPhaseComponents     flashedComposition;
   PVTComponents          composition;
   CompositionPetscVector concentrations;
   SaturationVector       saturations;
   PVTPhaseValues         density;
   ...
   ThreeVector minOilFlux;
   ThreeVector oilFlux;
   ThreeVector minGasFlux;
   ThreeVector gasFlux;
   ...
   double cflNumber = DefaultMaximumTimeStep * SecondsPerYear;
   double gasCflNumber;
   double oilCflNumber;
   double minimumGasCflNumber = DefaultMaximumTimeStep * SecondsPerYear;
   double minimumOilCflNumber = DefaultMaximumTimeStep * SecondsPerYear;
   double permNormal;
   double permPlane;
   ...
   unsigned int j;
   unsigned int k;
   unsigned int elementK;

   unsigned int minimumIGas = 0;
   unsigned int minimumJGas = 0;
   unsigned int minimumKGas = 0;

   unsigned int minimumIOil = 0;
   unsigned int minimumJOil = 0;
   unsigned int minimumKOil = 0;
      ...
      for ( j = formationGrid.firstJ (); j <= formationGrid.lastJ (); ++j ) {
         ...
         if ( FastcauldronSimulator::getInstance ().getMapElement ( i, j ).isValid ()) {
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               const LayerElement& layerElement = element.getLayerElement ();
               const Lithology* lithology = layerElement.getLithology ();
               const Saturation& saturation = saturations ( k, j, i );

               composition = concentrations ( k, j, i );
               oilFlux.zero ();
               gasFlux.zero ();

               if ( layerElement.isActive () and composition.sum () > ConcentrationLowerLimit ) {
                  composition *= m_defaultMolarMasses;

                  elementK = element.getK ();

                  getCoefficients ( layerElement, Basin_Modelling::Temperature, temperatureCoeffs );

                  // Need to add the capillary-pressure to this.
                  getCoefficients ( layerElement, Basin_Modelling::Pore_Pressure, phasePressureCoeffs );

                  getCoefficients ( layerElement, Basin_Modelling::Overpressure, overpressureCoeffs );
                  getCoefficients ( layerElement, Basin_Modelling::Hydrostatic_Pressure, hydrostaticPressureCoeffs );

                  getCoefficients ( layerElement, Basin_Modelling::VES_FP, vesCoeffs );
                  getCoefficients ( layerElement, Basin_Modelling::Max_VES, maxVesCoeffs );

                  // Convert pore-pressure to pascals from maga-pascals.
                  scale ( phasePressureCoeffs, 1.0e6 );

                  // Set the minimum pore-pressure to be the surface-pressure.
                  phasePressureCoeffs = maximum ( phasePressureCoeffs, 1.0e5 ); // Pascals
                  ...
                  porePressure = finiteElement.interpolate ( phasePressureCoeffs );
                  ...
                  // Convert to mol/m^3
                  flashedComposition /= m_defaultMolarMasses;

                  if ( flashedComposition.sum ( pvtFlash::VAPOUR_PHASE ) > ConcentrationLowerLimit ) {
                     finiteElement.setTensor ( permNormal, permPlane, permeability );
                     // permeability *= lithology->relativePermeability ( Saturation::GAS, saturation ( Saturation::GAS ));
                     gasCflNumber = computeElementCflNumber ( layerElement,
                                                              finiteElement,
                                                              phasePressureCoeffs, // Should be phase-pressure.
                                                              permeability,
                                                              density ( pvtFlash::VAPOUR_PHASE ),
                                                              viscosity ( pvtFlash::VAPOUR_PHASE ),
                                                              saturation ( Saturation::GAS ),
                                                              porosity.mixedProperty ());


                     gasFlux = computeMassFlux ( layerElement,
                                                 finiteElement,
                                                 phasePressureCoeffs, // Should be phase-pressure.
                                                 permeability,
                                                 density ( pvtFlash::VAPOUR_PHASE ),
                     ...
                     // if ( saturation ( Saturation::GAS ) > 0.0 ) {
                     //    gasFlux *= 1.0 / ( saturation ( Saturation::GAS ) * porosity.mixedProperty ());
                     // } else {
                     //    gasFlux.zero ();
                     // }

                  } else {
                     gasCflNumber = DefaultMaximumTimeStep * SecondsPerYear;
                  }


                  if ( flashedComposition.sum ( pvtFlash::LIQUID_PHASE ) > ConcentrationLowerLimit ) {
                     finiteElement.setTensor ( permNormal, permPlane, permeability );
                     // permeability *= lithology->relativePermeability ( Saturation::OIL, saturation ( Saturation::OIL ));
                     oilCflNumber = computeElementCflNumber ( layerElement,
                                                              finiteElement,
                                                              phasePressureCoeffs, // Should be phase-pressure.
                                                              permeability,
                                                              density ( pvtFlash::LIQUID_PHASE ),
                                                              viscosity ( pvtFlash::LIQUID_PHASE ),
                                                              saturation ( Saturation::OIL ),
                                                              porosity.mixedProperty ());

                     oilFlux = computeMassFlux ( layerElement,
                                                 finiteElement,
                                                 phasePressureCoeffs, // Should be phase-pressure.
                                                 permeability,
                                                 density ( pvtFlash::LIQUID_PHASE ),
                     ...
                     // if ( saturation ( Saturation::OIL ) > 0.0 ) {
                     //    oilFlux *= 1.0 / ( saturation ( Saturation::OIL ) * porosity.mixedProperty ());
                     // } else {
                     //    oilFlux.zero ();
                     // }

                     if ( isinf ( oilCflNumber )) {
                        ThreeVector flux = computeMassFlux ( layerElement,
                                                             finiteElement,
                                                             phasePressureCoeffs, // Should be phase-pressure.
                                                             permeability,
                                                             density ( pvtFlash::LIQUID_PHASE ),
                     ...
                     oilCflNumber = DefaultMaximumTimeStep * SecondsPerYear;
                  }

                  if ( minimumGasCflNumber > gasCflNumber ) {
                     minimumIGas = i;
                     minimumJGas = j;
                     minimumKGas = k;
                     minGasFlux = gasFlux;
                     minGasSat = saturation ( Saturation::GAS );
                  }

                  if ( minimumOilCflNumber > oilCflNumber ) {
                     minimumIOil = i;
                     minimumJOil = j;
                     minimumKOil = k;
                     minOilFlux = oilFlux;
                     minOilSat = saturation ( Saturation::OIL );
                     minPoro = porosity.mixedProperty ();
                  }

                  minimumGasCflNumber = NumericFunctions::Minimum ( minimumGasCflNumber, gasCflNumber );
                  minimumOilCflNumber = NumericFunctions::Minimum ( minimumOilCflNumber, oilCflNumber );
                  cflNumber = NumericFunctions::Minimum3 ( cflNumber, gasCflNumber, oilCflNumber );

#if 0
                  minimumGasCflNumber = NumericFunctions::Minimum ( minimumGasCflNumber, gasCflNumber );
                  minimumOilCflNumber = NumericFunctions::Minimum ( minimumOilCflNumber, oilCflNumber );

                  if ( cflNumber > gasCflNumber ) {
                     minimumIGas = i;
                     minimumJGas = j;
                     minimumKGas = k;
                     minGasFlux = gasFlux;
   ...
   if ( FastcauldronSimulator::getInstance ().getMcfHandler ().getDebugLevel () > 0 ) {
      std::string str = " Position for minimum : (gas)  %i   %i   %i   %e  %e   %e   %e  %e   %e\n Position for minimum : (oil)  %i   %i   %i   %e  %e   %e   %e  %e\n";
      PetscSynchronizedPrintf ( PETSC_COMM_WORLD, str.c_str (),
                                minimumIGas, minimumJGas, minimumKGas, minimumGasCflNumber, minGasSat, minGasFlux ( 1 ), minGasFlux ( 2 ), minGasFlux ( 3 ), minPoro,
                                minimumIOil, minimumJOil, minimumKOil, minimumOilCflNumber, minOilSat, minOilFlux ( 1 ), minOilFlux ( 2 ), minOilFlux ( 3 ));
      PetscSynchronizedFlush ( PETSC_COMM_WORLD );
   }

   double globalCflNumber;

   MPI_Allreduce ( &cflNumber, &globalCflNumber, 1, 
                   MPI_DOUBLE, MPI_MIN, PETSC_COMM_WORLD );

   return globalCflNumber;
}

//------------------------------------------------------------//

double ExplicitMultiComponentFlowSolver::computeCflNumber ( const Subdomain& subdomain ) {

   double cflNumber = DefaultMaximumTimeStep * SecondsPerYear;
   double formationCflNumber;
   ...
   while ( not iter.isDone ()) {
      formationCflNumber = computeCflNumber ( *iter );
      cflNumber = NumericFunctions::Minimum ( cflNumber, formationCflNumber );

      if ( FastcauldronSimulator::getInstance ().getMcfHandler ().getDebugLevel () > 0 ) {
         PetscPrintf ( PETSC_COMM_WORLD, " cfl value for formation %s: %e  %e\n", iter->getFormation ().getName ().c_str (), formationCflNumber, cflNumber );
      }
   ...
   if ( FastcauldronSimulator::getInstance ().getMcfHandler ().getDebugLevel () > 0 ) {
      PetscPrintf ( PETSC_COMM_WORLD, " cfl value for subdomdain:  %e\n", cflNumber );
   }
...
void ExplicitMultiComponentFlowSolver::applyOtgc ( SubdomainElement& element,
   ...
   OTGC6::SimulatorState otgcState ( subTimeStepStart, allSpecies, components );

   m_otgcSimulator->computeInterval ( otgcState, previousTemperature, currentTemperature, previousPorePressure, currentPorePressure, subTimeStepStart, subTimeStepEnd );
      ...
      for ( j = concentrationGrid.firstJ (); j <= concentrationGrid.lastJ (); ++j ) {
         ...
         if ( FastcauldronSimulator::getInstance ().getMapElement ( i, j ).isValid ()) {
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               applyOtgc ( element, concentration, immobiles, subTimeStepStart, subTimeStepEnd, lambdaStart, lambdaEnd );
            }
...
void ExplicitMultiComponentFlowSolver::applyOtgc ( Subdomain&   subdomain, 
   ...
   while ( not iter.isDone ()) {
      applyOtgc ( *iter, subTimeStepStart, subTimeStepEnd, lambdaStart, lambdaEnd );
      ++iter;
   }
         ...
         if ( FastcauldronSimulator::getInstance ().getMapElement ( i, j ).isValid ()) {
            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               if ( layerElement.isActive ()) {
                  ...
                  double elementVolume = volumeOfElement ( layerElement );
...
void ExplicitMultiComponentFlowSolver::assignGlobalSaturation ( Subdomain&        subdomain,
   ...
   while ( not iter.isDone ()) {
      ...
      // layerAveragedSaturations.restoreVector ( UPDATE_INCLUDING_GHOSTS );
...
void ExplicitMultiComponentFlowSolver::updateProgress ( const int status ) const {

   const char EscapeCharacter = 27; 

   // static char displayCharacters [ 4 ] = { '1', '2', '3', '4' };
   static char displayCharacters [ 4 ] = { '-', '\\', '|', '/' };

   cout << EscapeCharacter << "[1G" << flush;
   cout << displayCharacters [ status ];
   cout << EscapeCharacter << "[2G" << flush;
}

//------------------------------------------------------------//

void ExplicitMultiComponentFlowSolver::moveToStartOfline () const {
   const char EscapeCharacter = 27; 
   cout << EscapeCharacter << "[1G" << flush;
