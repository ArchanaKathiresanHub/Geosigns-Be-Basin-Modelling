...
protected:
  void SetUp() final
  {
    ...
    fdcLP = std::unique_ptr<fastDepthCalibration::FDCLithoProperties>(
          new fastDepthCalibration::FDCLithoProperties(mdl->stratigraphyManager(), mdl->lithologyManager()));
  }

  std::unique_ptr<fastDepthCalibration::FDCLithoProperties> fdcLP;
