            ...
            for ( k = formationGrid.firstK (); k <= formationGrid.lastK (); ++k ) {
               ...
               if ( m_interpolateSourceTerm && sourceTermInterpolator != 0 ) {
                  ...
                  for( l = 0; l < NumberOfPVTComponents; ++ l ) {     
                     pvtFlash::ComponentId component = static_cast<pvtFlash::ComponentId>( l );
...
void DarcyCalculations::computeSaturation ( const SubdomainElement&        element,
   ...
   totalComposition = phaseComposition.getPhaseComponents ( pvtFlash::VAPOUR_PHASE );
   totalComposition += phaseComposition.getPhaseComponents ( pvtFlash::LIQUID_PHASE );
   ...
   if ( vapourCompositionSum > HcConcentrationLowerLimit ) {
      vapourSaturation = concentrationSum * vapourFraction * molarMass ( pvtFlash::VAPOUR_PHASE ) / vapourCompositionSum / density ( pvtFlash::VAPOUR_PHASE );
   } else {
      vapourSaturation = 0.0;
   }

   if ( liquidCompositionSum > HcConcentrationLowerLimit ) {
      liquidSaturation = concentrationSum * ( 1.0 - vapourFraction ) * molarMass ( pvtFlash::LIQUID_PHASE ) / liquidCompositionSum / density ( pvtFlash::LIQUID_PHASE );
   } else {
      liquidSaturation = 0.0;
   }
