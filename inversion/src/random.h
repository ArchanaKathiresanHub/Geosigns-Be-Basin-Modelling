...
static unsigned long gla_mt[NN]; // the array for the state vector  
static int gl_mti=NN+1; // gl_mti==N+1 means gla_mt[N] is not initialized 
...
float genrand()
{
    ...
    if (gl_mti >= NN) { // generate N words at one time 
        int kk;

        if (gl_mti == NN+1)   // if sgenrand() has not been called,
            sgenrand(4357); // a default initial seed is used   

        for (kk=0;kk<NN-MM;kk++) {
            y = (gla_mt[kk]&UPPER_MASK)|(gla_mt[kk+1]&LOWER_MASK);
            gla_mt[kk] = gla_mt[kk+MM] ^ (y >> 1) ^ mag01[y & 0x1];
        }
        for (;kk<NN-1;kk++) {
            y = (gla_mt[kk]&UPPER_MASK)|(gla_mt[kk+1]&LOWER_MASK);
            gla_mt[kk] = gla_mt[kk+(MM-NN)] ^ (y >> 1) ^ mag01[y & 0x1];
        }
        y = (gla_mt[NN-1]&UPPER_MASK)|(gla_mt[0]&LOWER_MASK);
        gla_mt[NN-1] = gla_mt[MM-1] ^ (y >> 1) ^ mag01[y & 0x1];
