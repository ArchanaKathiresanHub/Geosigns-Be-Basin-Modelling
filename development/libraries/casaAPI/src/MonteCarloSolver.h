...
{  
   ...
   public:
      ...
      /// @brief Get CDF (cumulative density function) for P10-P90 quantiles
      /// @return vector size numObs x 9 which keeps CDF values for 10-20-30-40-50..-90% probability for each observable
      virtual const std::vector<std::vector<double> > & p10p90CDF() const = 0;

      /// @brief Get number of samplings generated by MC simulation
      /// @return number of samplings
      virtual size_t samplingsNumber() const = 0;

      /// @brief Get RMSE value for the i-th sampling
      /// @param i zero-based index of the sampling
      ...
      virtual const RunCase * samplingPoint( size_t i ) const = 0;
