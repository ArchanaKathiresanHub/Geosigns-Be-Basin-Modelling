...
public:
         ...
         DistributedGridMap (const Grid * grid, unsigned int depth, double undefinedValue);
	 	 /// Create a constant GridMap from the given value
     	 DistributedGridMap (const Parent * owner, unsigned int childIndex, const Grid * grid, double value, unsigned int depth = 1);
    	 /// Create a GridMap from the given grid with the given array of values
	     DistributedGridMap (const Parent * owner, unsigned int childIndex, const Grid * grid, double undefinedValue, unsigned int depth, 
		 float *** values);
	     /// Create a GridMap from the two given GridMap objects after elementwise processing by the specified operator function.
	     DistributedGridMap (const Parent * owner, unsigned int childIndex, const GridMap * operand1, const GridMap * operand2, 
		 BinaryOperator binaryOperator);
	     /// Create a GridMap from the given GridMap after elementwise processing by the specified operator function.
	     DistributedGridMap (const Parent * owner, unsigned int childIndex, const GridMap * operand, UnaryOperator unaryOperator);
	     ...
	     /// Create a GridMap from the two given GridMap objects after elementwise processing by the specified binary function object.
	     DistributedGridMap (const Parent * owner, unsigned int childIndex, const GridMap * operand1, UnaryFunctor& unaryFunctor );
...
private:
	...
	bool findLowResElementCoordinates(const unsigned int HighResI, const unsigned int HighResJ, const unsigned int depth,
                                	     const DistributedGridMap *lowResMap,  const DistributedGrid *lowResGrid,  
                                	     const DistributedGrid *HighResGrid, 
                                	     unsigned int lowResElementCoordinatesInHighRes[], double nodalValuesInLowResElement[], bool useGhostNodesInLowRes);
	//utility for findLowResElementCoordinates
	bool isHighResNodeInLowResElement(const unsigned int &HighResI, const unsigned int &HighResJ, unsigned int lowResElementCoordinatesInHighRes[]);
	bool isHighResNodeInLowResGrid(const unsigned int &HighResI, const unsigned int &HighResJ, const GridMap *lowResMap,
