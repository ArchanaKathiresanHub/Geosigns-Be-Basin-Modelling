                                       ...
                                       double& capSealStrength_H2O_Gas, double& capSealStrength_H2O_Oil) const
   {
      ...
      if ( m_isLegacy )
      {
         if ( composition[0].getWeight() > 0.0 ) {
            double T_c_K = pvtFlash::criticalTemperatureAccordingToLiMixingRuleWithLumping(composition[0].
                                                                                           getWeights(), gorm);
            capSealStrength_H2O_Gas = CBMGenerics::capillarySealStrength::capSealStrength_H2O_HC(m_lithProps[0],
                                                                                                 m_lithFracs[0], m_mixModel[0], m_permeability[0], m_sealFluidDensity, composition[0].getDensity(),
                                                                                                 T_K, T_c_K);
         }

         if ( composition[1].getWeight() > 0.0 ) {
            double T_c_K = pvtFlash::criticalTemperatureAccordingToLiMixingRuleWithLumping(composition[1].
                                                                                           getWeights(), gorm);
            capSealStrength_H2O_Oil = CBMGenerics::capillarySealStrength::capSealStrength_H2O_HC(m_lithProps[0],
