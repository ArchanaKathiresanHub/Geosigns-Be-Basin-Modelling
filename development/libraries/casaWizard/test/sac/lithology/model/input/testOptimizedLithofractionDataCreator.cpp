...
TEST( OptimizedLithofractionDataCreator, testReadLithofractionFile )
{
  ...
  casaWizard::sac::lithology::OptimizedLithofractionDataCreator dataCreator{scenario, iterationPath};

  dataCreator.read();

  const QVector<casaWizard::sac::lithology::OptimizedLithofraction> optimizedLithofractions = lithofractionManager.optimizedLithofractions();
  ...
  int index = 0;
  for (const casaWizard::sac::lithology::OptimizedLithofraction& optimizedActual : optimizedLithofractions)
  {
