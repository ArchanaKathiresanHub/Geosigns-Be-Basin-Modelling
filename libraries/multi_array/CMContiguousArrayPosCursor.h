#ifndef _MULTI_ARRAY_CMCONTIGUOUSARRAYPOSCURSOR_H_
#define _MULTI_ARRAY_CMCONTIGUOUSARRAYPOSCURSOR_H_

#include "multi_array/ContiguousSteppedCursor.h"

#define CMFORWARDPOSCURSOR CMContiguousArrayPosCursorBase
...
class CMContiguousArrayPosCursor: 
  public CMContiguousArrayPosCursorBase<DIM,ENTRY>
{
public:
  ...
  inline CMContiguousArrayPosCursor(const CloseOpenInterval<DIM,int>& domain,
    const Tuple<DIM,int>& arraySteps, ENTRY* data):
    CMContiguousArrayPosCursorBase<DIM,ENTRY>( ContiguousSteppedCursor<ENTRY>( 
  ...
  inline CMContiguousArrayPosCursor(const CloseOpenInterval<DIM,int>& domain,
    ...
    CMContiguousArrayPosCursorBase<DIM,ENTRY>( ContiguousSteppedCursor<ENTRY>( 
      domain.volume(), 1, data, CMStrided2IndexMap<DIM>(domain.size()).
