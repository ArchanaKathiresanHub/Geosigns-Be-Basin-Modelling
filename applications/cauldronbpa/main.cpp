...
int main( int argc, char ** argv )
{
   ...
   for (i = 1; i < argc; i++)
   {
      ...
      if (strncmp (argv[i], "-verbose", MAX (arglen, 2)) == 0)
      {
         cbVerbose = true;
      }
      ...
      else if (strncmp (argv[i], "-server", MAX (arglen, 2)) == 0)
      {
         myServer = argv[++i];
      }
      ...
      {
         cerr << "Usage: " << argv[0] << " [-server servername] [-verbose] [-testing] [-timing] [projectfile]" << endl;
         return -1;
      }
   }
   ...
   if (projectFileName)
      cw->loadProject (projectFileName);
   else
   {
      cw->show ();
      cw->startLogin ();
