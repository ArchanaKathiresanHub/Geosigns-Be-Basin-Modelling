...
class CONTIGUOUSSTEPPEDFORWARDCURSOR
{
private:
...
public:
  ...
  inline CONTIGUOUSSTEPPEDFORWARDCURSOR(int size, int step, CONST ENTRY* data):
    ...
    m_finish( (CONST char*)data + m_charStep * size )
  {}
  ...
  /// @param[in] step: the step between consecutive entries.  For instance, if only every second element
  /// is wanted, step must be equal to 2.
  inline CONTIGUOUSSTEPPEDFORWARDCURSOR(int size, int step, CONST ENTRY* data, int pos):
    m_charStep( step * sizeof(ENTRY) ),
    m_here( (CONST char*)data ),
    m_finish( (CONST char*)data + m_charStep * size )
  {}
